...
static double central_ang(double reflat, double reflon, double obslat, double obslon)
{
/*
...
*
* Using the following Product to Sum Trigonometric formula:
*
...
* 
* 	acos ( 
*			cos(obslon - reflon) * ( cos(reflat - obslat) + cos(reflat + obslat) ) * 0.5 +
*			( cos(reflat - obslat) - cos(reflat + obslat) ) * 0.5
*		 )
*
* Further simplification gives:
*
*	acos ( 0.5 * (
*			cos(obslon - reflon)cos(reflat - obslat) + cos(reflat - obslat) + 
*			cos(obslon - reflon)cos(reflat + obslat) - cos(reflat + obslat)
*		         )
*		 )
*
...
*
*	acos ( 0.5 * (
*			( cos(obslon - reflon) + 1 )cos(reflat - obslat) +
*			( cos(obslon - reflon) - 1 )cos(reflat + obslat)
*				 )
*		 )
*
* By doing this we have reduced 3 cosines and 2 sines to 3 cosines which should reduce the effect in the difference between trigonometric function implementations
*/
  double cd = cos(obslon-reflon);
  double amb = cos(reflat - obslat) ;
  double apb =  cos(reflat + obslat);

  return (double) ( acos(0.5 * ( (cd + 1) * amb + (cd - 1) * apb ) ) ) ;
}

static double
rad(double reflat, double reflon, double refdeg, double obslat, double obslon) 
{
  return (double) (central_ang(reflat,reflon,obslat,obslon) <= refdeg ) ;
}

static double
distance(double obslat, double obslon, double reflat, double reflon)
{
  return (double) (R_Earth * central_ang(reflat,reflon,obslat,obslon) ) ;
}

static double
dist(double reflat, double reflon, double refdist_km, double obslat, double obslon)
{
  return (double) (R_Earth_km * central_ang(reflat,reflon,obslat,obslon) <= (refdist_km) ) ;
}

static double
km(double reflat, double reflon, double obslat, double obslon)
{
  return (double) (R_Earth_km * central_ang(reflat,reflon,obslat,obslon) ) ;
}
