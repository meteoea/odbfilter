...
{ \
 Type *Ptr = NULL; \
 oml_set_lockid(&ODB_global_mylock[0]); \
 ...
 oml_unset_lockid(&ODB_global_mylock[0]); \
 return Ptr; \
} \
...
{ \
 Packed_DS *PDS; \
 ALLOC(PDS,1); \
 oml_set_lockid(&ODB_global_mylock[1]); { \
 ...
 } oml_unset_lockid(&ODB_global_mylock[1]); \
 return PDS; \
}
#endif
...
{ \
 ...
 int PoolNo = P->PoolNo; \
 int Handle = P->Handle; \
 oml_set_lockid(&ODB_global_mylock[2]); \
 if (!P->Is_loaded && P->IO_method == 5) { \
   if (!DCA_getsize(Handle, ODB_LABEL, #Table, PoolNo, &P->Nrows, &P->Ncols)) { \
     int iret; \
     (void) DCA_alloc(Handle, ODB_LABEL, #Table, &iret); \
...
#define PostLoadTable(Table) \
 ...
 oml_unset_lockid(&ODB_global_mylock[2]); \
 return Nbytes; \
}
...
{ \
 int Nbytes = 0; \
 TABLE_##Table *P = T; \
 oml_set_lockid(&ODB_global_mylock[3]); \
 if (P->Is_loaded && P->IO_method != 5) { \
  int rc; \
  int PoolNo = P->PoolNo; \
  int Handle = P->Handle; \
...
return_Nbytes: \
 oml_unset_lockid(&ODB_global_mylock[3]); \
 return Nbytes; \
}
