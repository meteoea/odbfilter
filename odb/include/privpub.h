...
typedef oml_lock_t o_lock_t;

#ifdef _OML_H_

/* My thread-id [1 .. $OMP_NUM_THREADS] as in oml_mod.F90 */
static int odb_get_thread_id() {
  return oml_my_thread();
}

static int odb_get_max_threads() {
  return oml_get_max_threads();
}

static int odb_get_num_threads() {
...
#define DEF_IT    int it = odb_get_thread_id()
#define IT        (it-1)
#define DEF_INUMT int inumt = odb_get_max_threads()
