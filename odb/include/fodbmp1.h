...
#define GEN_TYPE          REAL(KIND=JPRD)
...
if (nreq > 0) then
     call MPL_WAIT(krequest=ireq(1:nreq), cdstring=GEN_XCHANGE1_NAME//' : wait for send data')
end if
if (nreq_zerodata > 0) then
    call MPL_WAIT(krequest=ireq_zerodata(1:nreq_zerodata), cdstring=GEN_XCHANGE1_NAME//' : wait for send zerodata')
end if
