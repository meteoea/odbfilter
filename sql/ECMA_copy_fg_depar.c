...
#define ODB_CONSIDER_TABLES "/hdr/body/update_1/"
...
#define update_1_ROW ((double)(K2 + 1))
...
typedef struct {
  ...
  ODBMAC_VIEW_TABLEDECL_WITH_INDEX(update_1);
  uint can_UPDATE[RNDUP_DIV(2,MAXBITS)];
} VIEW_copy_fg_depar;

PRIVATE int nV_copy_fg_depar_TAG = 2;
PRIVATE int nV_copy_fg_depar_MEM = 0;

PRIVATE const ODB_Tags V_copy_fg_depar_TAG[2] = {
  /* === SELECT-symbols (count = 2) === */
  { "pk9real:fg_depar@body", 0, 0, NULL } ,
  { "pk9real:hires@update_1", 0, 0, NULL } ,
...
PRIVATE int nV_copy_fg_depar_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_copy_fg_depar_PREPTAG[3] = {
  /* Prepared tags for faster codb_getnames() */
  { (preptag_name | preptag_extname), 30,
    ";fg_depar@body;hires@update_1;" },
  { (preptag_type | preptag_exttype), 17,
    ";pk9real;pk9real;" },
  {  preptag_tblname, 22,
    ";@hdr;@body;@update_1;" },
};
...
PRIVATE void
Ccl_V_copy_fg_depar(void *T)
{
  ODBMAC_CCL_V_PRE(copy_fg_depar);
  FREEINDEX(body);
  FREEINDEX(update_1);
}

PRIVATE int
PrS_V_copy_fg_depar(FILE *do_trace,
  VIEW_copy_fg_depar *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, unsigned int BmapIdx0[]
  /* TABLE 'body' */ , int N1, unsigned int BmapIdx1[]
  /* TABLE 'update_1' */ , int N2, unsigned int BmapIdx2[])
{
  ...
  linkoffset_t *Pupdate_1_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(update_1));
  linklen_t *Pupdate_1_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(update_1));
  DRHOOK_START(PrS_V_copy_fg_depar);
  ODBMAC_PEINFO_SETUP();
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      ...
      { /* if-block start */
        for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'body' : weight = 361.000382 */
          int K2_lo = Pupdate_1_off[K0];
          int K2;
              tmpcount++; /* TABLE 'update_1' : weight = 361.000384 */
        } /* TABLE 'body' */
      } /* if-block end */
    } /* TABLE 'hdr' */
...
PRIVATE int
PoS_V_copy_fg_depar(FILE *do_trace,
  const VIEW_copy_fg_depar *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, const unsigned int BmapIdx0[]
  /* TABLE 'body' */ , int N1, const unsigned int BmapIdx1[], int Index_body[]
  /* TABLE 'update_1' */ , int N2, const unsigned int BmapIdx2[], int Index_update_1[])
{
  double *Addr = (PEinfo && P->USD_symbols > 0) ? PEinfo->addr : NULL;
  int PE, PEstart = 1;
  int PEend = PEinfo ? PEinfo->npes : PEstart;
  ...
  linkoffset_t *Pupdate_1_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(update_1));
  linklen_t *Pupdate_1_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(update_1));
  DRHOOK_START(PoS_V_copy_fg_depar);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      ...
      { /* if-block start */
        for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'body' : weight = 361.000382 */
          int K2_lo = Pupdate_1_off[K0];
          int K2 = K2_lo;
          K2 = K2_lo + (K1 - K1_lo); { /* TABLE 'update_1' (due to ALIGN with 'body') : weight = 361.000384 */
            Index_body[tmpcount] = K1;
            Index_update_1[tmpcount] = K2;
            tmpcount++;
          } /* TABLE 'update_1' */
        } /* TABLE 'body' */
      } /* if-block end */
...
PRIVATE int
Sel_V_copy_fg_depar(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_copy_fg_depar *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(copy_fg_depar, "hdr,body,update_1");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_copy_fg_depar);
  ...
  ODBMAC_VIEW_DELAYED_LOAD(update_1);
  ODBMAC_TRACE_SELVIEW_0();
  ODBMAC_TRACE_SELVIEW_1();
  { /* Start of pre- & post-select block */
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    ALLOCBITMAPINDEX(update_1);
    CountPrS = PrS_V_copy_fg_depar(do_trace, P, it, PEinfo
      , P->T_hdr->Nrows, P->BitmapIndex_hdr
      , P->T_body->Nrows, P->BitmapIndex_body
      , P->T_update_1->Nrows, P->BitmapIndex_update_1);
    ODBMAC_TRACE_SELVIEW_POST();
    ALLOCINDEX(body, CountPrS);
    ALLOCINDEX(update_1, CountPrS);
    ODBMAC_TRACE_SELVIEW_2();
    if (CountPrS > 0) {
      CountPoS = PoS_V_copy_fg_depar(do_trace, P, it, PEinfo
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
        , P->T_body->Nrows, P->BitmapIndex_body, P->Index_body
        , P->T_update_1->Nrows, P->BitmapIndex_update_1, P->Index_update_1); }
    else { CountPoS = CountPrS; }
    FREEBITMAPINDEX(hdr);
    FREEBITMAPINDEX(body);
    FREEBITMAPINDEX(update_1);
  } /* End of pre-& post-select block */
  ODBMAC_TRACE_SELVIEW_LAST();
  ODBMAC_ERRMSG_SELVIEW(copy_fg_depar);
  ODB_debug_print_index(stdout, "copy_fg_depar", P->PoolNo, CountPrS, 2
        , "body", P->Index_body, P->T_body, P->T_body->Nrows
        , "update_1", P->Index_update_1, P->T_update_1, P->T_update_1->Nrows);
  P->Nrows = CountPrS;
  DRHOOK_END(CountPrS);
  return CountPrS;
}
...
PRIVATE int
dGet_V_copy_fg_depar(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_copy_fg_depar);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(ECMA, d, 1, copy_fg_depar, pk9real, K1, K2, body, D, fg_depar, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, copy_fg_depar, pk9real, K1, K2, update_1, D, hires, DATATYPE_REAL8, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_copy_fg_depar(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_copy_fg_depar);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, copy_fg_depar, pk9real, K1, K2, body, fg_depar, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, copy_fg_depar, pk9real, K1, K2, update_1, hires, D, DATATYPE_REAL8, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_copy_fg_depar(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(copy_fg_depar); }

#define Swapout_V_copy_fg_depar Ccl_V_copy_fg_depar

PRIVATE int
Sql_V_copy_fg_depar(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_copy_fg_depar(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(copy_fg_depar);
  return filled;
}

PRIVATE int *
SortKeys_V_copy_fg_depar(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(copy_fg_depar); }

PRIVATE int 
UpdateInfo_V_copy_fg_depar(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(copy_fg_depar); }

PRIVATE int *
GetIndex_V_copy_fg_depar(void *V, const char *Table, int *Nidx)
{
  VIEW_copy_fg_depar *P = V;
  int Dummy = 0;
  int *Nlen = Nidx ? Nidx : &Dummy;
  ODBMAC_GETINDEX(body);
  ODBMAC_GETINDEX(update_1);
  return NULL;
}

PRIVATE int
PutIndex_V_copy_fg_depar(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
  /* *** Warning: This is a VERY DANGEROUS routine -- if misused !!! */
  VIEW_copy_fg_depar *P = V;
  int rc = 0;
  ODBMAC_PUTINDEX(body);
  ODBMAC_PUTINDEX(update_1);
  return rc;
}

PRIVATE void
PEinfo_V_copy_fg_depar(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(copy_fg_depar); }

PRIVATE void *
Init_V_copy_fg_depar(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_copy_fg_depar *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_copy_fg_depar);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 0, 0);   /* pk9real:fg_depar@body */
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 1, 1);   /* pk9real:hires@update_1 */
  {
    ODB_Pool *p = Pool;
    ODBMAC_ASSIGN_TABLEDATA(hdr);
    ODBMAC_ASSIGN_TABLEDATA(body);
    ODBMAC_ASSIGN_TABLEDATA(update_1);
  }
  NULLIFY_INDEX(body);
  NULLIFY_INDEX(update_1);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "copy_fg_depar"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_copy_fg_depar_TAG;
    pfcom->preptags = V_copy_fg_depar_PREPTAG;
    pfcom->ntag = nV_copy_fg_depar_TAG;
    pfcom->npreptag = nV_copy_fg_depar_PREPTAG;
    pfcom->nmem = nV_copy_fg_depar_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_copy_fg_depar;
    pfcom->swapout = Swapout_V_copy_fg_depar;
    pfcom->dim = Dim_V_copy_fg_depar;
    pfcom->sortkeys = SortKeys_V_copy_fg_depar;
    pfcom->update_info = UpdateInfo_V_copy_fg_depar;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_copy_fg_depar;
    pfcom->putindex = PutIndex_V_copy_fg_depar;
    pfcom->peinfo = PEinfo_V_copy_fg_depar;
    pfcom->select = Sel_V_copy_fg_depar;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_copy_fg_depar;
    pfcom->dget = dGet_V_copy_fg_depar;
    pfcom->dput = dPut_V_copy_fg_depar;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_copy_fg_depar;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_copy_fg_depar;
...
/* *************** End of VIEW "copy_fg_depar" *************** */

PUBLIC ODB_Funcs *
Anchor2ECMA_copy_fg_depar(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_copy_fg_depar *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_copy_fg_depar);
  if (!P) P = Init_V_copy_fg_depar(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
