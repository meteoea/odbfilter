...
#define INDEX_Timeslot_all_EXPR(i) T_indexXtimeslot[i] /* 'pk1int:timeslot@index' */


PRIVATE double USD_otype_ECMA = 0; /* $otype */
PRIVATE double USD_vno_u_ECMA = 0; /* $vno_u */
PRIVATE double USD_vno_v_ECMA = 0; /* $vno_v */
PRIVATE double USD_tslot_ECMA = 0; /* $tslot */
...
/* *************** VIEW "select_wind_obstype_varno_tslot" *************** */
...
typedef struct {
  ...
  uint can_UPDATE[RNDUP_DIV(7,MAXBITS)];
} VIEW_select_wind_obstype_varno_tslot;

PRIVATE int nV_select_wind_obstype_varno_tslot_TAG = 15;
PRIVATE int nV_select_wind_obstype_varno_tslot_MEM = 0;

PRIVATE const ODB_Tags V_select_wind_obstype_varno_tslot_TAG[15] = {
  /* === SELECT-symbols (count = 7) === */
  { "pk9real:obsvalue@body", 0, 0, NULL } ,
  { "Bitfield:datum_status.active@body", 0, 0, NULL } ,
  { "string:statid@hdr", 0, 0, NULL } ,
  ...
  /* === WHERE-symbols (count = 3) === */
  { "pk1int:obstype@hdr", 0, 0, NULL },
  { "pk1int:varno@body", 0, 0, NULL },
  { "pk1int:timeslot@index", 0, 0, NULL },
  /* === ORDERBY-symbols (count = 5) === */
  { "string:statid@hdr", 0, 3, NULL },
  { "pk9real:lat@hdr", 0, 4, NULL },
  { "pk9real:lon@hdr", 0, 5, NULL },
  { "yyyymmdd:date@hdr", 0, 6, NULL },
  { "hhmmss:time@hdr", 0, 7, NULL },
  /* === UNIQUEBY-symbols (count = 0) === */
};

PRIVATE int nV_select_wind_obstype_varno_tslot_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_wind_obstype_varno_tslot_PREPTAG[3] = {
  /* Prepared tags for faster codb_getnames() */
  { (preptag_name | preptag_extname), 85,
    ";obsvalue@body;datum_status.active@body;statid@hdr;"
    "lat@hdr;lon@hdr;date@hdr;time@hdr;" },
  { (preptag_type | preptag_exttype), 57,
    ";pk9real;Bitfield;string;pk9real;pk9real;yyyymmdd;"
    "hhmmss;" },
  {  preptag_tblname, 19,
    ";@index;@hdr;@body;" },
};
...
PRIVATE void
Ccl_V_select_wind_obstype_varno_tslot(void *T)
{
  ODBMAC_CCL_V_PRE(select_wind_obstype_varno_tslot);
  FREEINDEX(hdr);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_wind_obstype_varno_tslot(FILE *do_trace,
  VIEW_select_wind_obstype_varno_tslot *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk1int T_indexXtimeslot[])
{
  double lc_USD_otype = 0;
  double lc_USD_vno_u = 0;
  double lc_USD_vno_v = 0;
  double lc_USD_tslot = 0;
  ...
  DRHOOK_START(PrS_V_select_wind_obstype_varno_tslot);
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(ECMA, vno_u, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(ECMA, vno_v, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(ECMA, tslot, "select_wind_obstype_varno_tslot");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_wind_obstype_varno_tslot");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vno_u, "select_wind_obstype_varno_tslot");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vno_v, "select_wind_obstype_varno_tslot");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, tslot, "select_wind_obstype_varno_tslot");
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'index' : weight = 1.000003 */
      ...
      int K1;
      double tmp10 = INDEX_Timeslot_all_EXPR(K0);
      int cond0 = (tmp10 == lc_USD_tslot);
      if (cond0 == 0) continue;
      K1 = K1_lo; { /* TABLE 'hdr' (due to ONELOOPER with 'index') : weight = 3.000004 */
        int K2_lo = Pbody_off[K1];
        int K2_hi = K2_lo + Pbody_len[K1];
        int K2;
        double tmp7 = HDR_Obstype_all_EXPR(K1);
        int cond1 = (tmp7 == lc_USD_otype);
        if (cond1 == 0) continue;
        for (K2=K2_lo; K2<K2_hi; K2++) { /* TABLE 'body' : weight = 361.000382 */
          double tmp8 = BODY_Varno_all_EXPR(K2);
          int cond2 = ((tmp8 == lc_USD_vno_u) || 
            (tmp8 == lc_USD_vno_v));
          if (cond2 != 0)
          { /* if-block start */
...
PRIVATE int
PoS_V_select_wind_obstype_varno_tslot(FILE *do_trace,
  const VIEW_select_wind_obstype_varno_tslot *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk1int T_indexXtimeslot[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_wind_obstype_varno_tslot);
  ODBMAC_LC_GETVAL(ECMA, otype, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(ECMA, vno_u, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(ECMA, vno_v, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(ECMA, tslot, "select_wind_obstype_varno_tslot");
...
PRIVATE int
Sel_V_select_wind_obstype_varno_tslot(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_wind_obstype_varno_tslot *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_wind_obstype_varno_tslot, "index,hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_wind_obstype_varno_tslot);
  ...
  { /* Start of pre- & post-select block */
    const pk1int *TmpVec_7 = UseDSlong(P->T_hdr, ECMA, pk1int, P->T_hdr->obstype);
    const pk1int *TmpVec_8 = UseDSlong(P->T_body, ECMA, pk1int, P->T_body->varno);
    const pk1int *TmpVec_10 = UseDSlong(P->T_index, ECMA, pk1int, P->T_index->timeslot);
    ALLOCBITMAPINDEX(index);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_wind_obstype_varno_tslot(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_wind_obstype_varno_tslot(do_trace, P, it, PEinfo
        , P->T_index->Nrows, P->BitmapIndex_index
  ...
  ODBMAC_ERRMSG_SELVIEW(select_wind_obstype_varno_tslot);
  ODB_debug_print_index(stdout, "select_wind_obstype_varno_tslot", P->PoolNo, CountPrS, 2
...
PRIVATE int
dGet_V_select_wind_obstype_varno_tslot(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_wind_obstype_varno_tslot);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(ECMA, d, 1, select_wind_obstype_varno_tslot, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_wind_obstype_varno_tslot, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  Call_GatherGet_VIEW(ECMA, d, 3, select_wind_obstype_varno_tslot, string, K1, K2, hdr, D, statid, DATATYPE_STRING, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 4, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, D, lat, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 5, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, D, lon, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 6, select_wind_obstype_varno_tslot, yyyymmdd, K1, K2, hdr, D, date, DATATYPE_YYYYMMDD, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 7, select_wind_obstype_varno_tslot, hhmmss, K1, K2, hdr, D, time, DATATYPE_HHMMSS, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_wind_obstype_varno_tslot(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_wind_obstype_varno_tslot);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_wind_obstype_varno_tslot, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_wind_obstype_varno_tslot, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  Call_ScatterPut_VIEW(ECMA, d, 3, select_wind_obstype_varno_tslot, string, K1, K2, hdr, statid, D, DATATYPE_STRING, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 4, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, lat, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 5, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, lon, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 6, select_wind_obstype_varno_tslot, yyyymmdd, K1, K2, hdr, date, D, DATATYPE_YYYYMMDD, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 7, select_wind_obstype_varno_tslot, hhmmss, K1, K2, hdr, time, D, DATATYPE_HHMMSS, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_wind_obstype_varno_tslot(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_wind_obstype_varno_tslot); }

#define Swapout_V_select_wind_obstype_varno_tslot Ccl_V_select_wind_obstype_varno_tslot

PRIVATE int
Sql_V_select_wind_obstype_varno_tslot(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_wind_obstype_varno_tslot(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 7;
  static const int ColAux[7] = {1,2,3,4,5,6,7};
  ODBMAC_COPY_COLAUX(select_wind_obstype_varno_tslot);
  return filled;
}

PRIVATE int *
SortKeys_V_select_wind_obstype_varno_tslot(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_wind_obstype_varno_tslot); }

PRIVATE int 
UpdateInfo_V_select_wind_obstype_varno_tslot(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_wind_obstype_varno_tslot); }

PRIVATE int *
GetIndex_V_select_wind_obstype_varno_tslot(void *V, const char *Table, int *Nidx)
{
  VIEW_select_wind_obstype_varno_tslot *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_wind_obstype_varno_tslot(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_wind_obstype_varno_tslot(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_wind_obstype_varno_tslot); }

PRIVATE void *
Init_V_select_wind_obstype_varno_tslot(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_wind_obstype_varno_tslot *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_wind_obstype_varno_tslot);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 7;
  P->Nrows = 0;
  P->USD_symbols = 4; /* In SELECT = 0 ; In WHERE = 4 */
  P->Replicate_PE = 0;
  P->Npes = 0;
  P->NrowVec = NULL;
  P->NrowOffset = NULL;
  ...
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 2, 2);   /* string:statid@hdr */
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 3, 3);   /* pk9real:lat@hdr */
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 4, 4);   /* pk9real:lon@hdr */
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 5, 5);   /* yyyymmdd:date@hdr */
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 6, 6);   /* hhmmss:time@hdr */
  {
    ODB_Pool *p = Pool;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_wind_obstype_varno_tslot"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_wind_obstype_varno_tslot_TAG;
    pfcom->preptags = V_select_wind_obstype_varno_tslot_PREPTAG;
    pfcom->ntag = nV_select_wind_obstype_varno_tslot_TAG;
    pfcom->npreptag = nV_select_wind_obstype_varno_tslot_PREPTAG;
    pfcom->nmem = nV_select_wind_obstype_varno_tslot_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_wind_obstype_varno_tslot;
    pfcom->swapout = Swapout_V_select_wind_obstype_varno_tslot;
    pfcom->dim = Dim_V_select_wind_obstype_varno_tslot;
    pfcom->sortkeys = SortKeys_V_select_wind_obstype_varno_tslot;
    pfcom->update_info = UpdateInfo_V_select_wind_obstype_varno_tslot;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_wind_obstype_varno_tslot;
    pfcom->putindex = PutIndex_V_select_wind_obstype_varno_tslot;
    pfcom->peinfo = PEinfo_V_select_wind_obstype_varno_tslot;
    pfcom->select = Sel_V_select_wind_obstype_varno_tslot;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_wind_obstype_varno_tslot;
    pfcom->dget = dGet_V_select_wind_obstype_varno_tslot;
    pfcom->dput = dPut_V_select_wind_obstype_varno_tslot;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_wind_obstype_varno_tslot;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_wind_obstype_varno_tslot;
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_wind_obstype_varno_tslot", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$vno_u", "select_wind_obstype_varno_tslot", it, USD_vno_u_ECMA);
    Pool->add_var("ECMA", "$vno_v", "select_wind_obstype_varno_tslot", it, USD_vno_v_ECMA);
    Pool->add_var("ECMA", "$tslot", "select_wind_obstype_varno_tslot", it, USD_tslot_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}

/* *************** End of VIEW "select_wind_obstype_varno_tslot" *************** */

PUBLIC ODB_Funcs *
Anchor2ECMA_select_wind_obstype_varno_tslot(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_wind_obstype_varno_tslot *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_wind_obstype_varno_tslot);
  if (!P) P = Init_V_select_wind_obstype_varno_tslot(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
