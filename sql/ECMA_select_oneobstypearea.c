...
PRIVATE double USD_varno_ECMA = 0; /* $varno */
PRIVATE double USD_lonmin_ECMA = 0; /* $lonmin */
PRIVATE double USD_lonmax_ECMA = 0; /* $lonmax */
PRIVATE double USD_latmin_ECMA = 0; /* $latmin */
PRIVATE double USD_latmax_ECMA = 0; /* $latmax */
PRIVATE double USD_presmin_ECMA = 0; /* $presmin */
PRIVATE double USD_presmax_ECMA = 0; /* $presmax */
  ...
  const pk9real T_bodyXvertco_reference_1[])
{
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_oneobstypearea");
  ODBMAC_LC_GETVAL(ECMA, varno, "select_oneobstypearea");
  ODBMAC_LC_GETVAL(ECMA, lonmin, "select_oneobstypearea");
  ODBMAC_LC_GETVAL(ECMA, lonmax, "select_oneobstypearea");
  ODBMAC_LC_GETVAL(ECMA, latmin, "select_oneobstypearea");
  ODBMAC_LC_GETVAL(ECMA, latmax, "select_oneobstypearea");
  ODBMAC_LC_GETVAL(ECMA, presmin, "select_oneobstypearea");
  ODBMAC_LC_GETVAL(ECMA, presmax, "select_oneobstypearea");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_oneobstypearea");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, varno, "select_oneobstypearea");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, lonmin, "select_oneobstypearea");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, lonmax, "select_oneobstypearea");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, latmin, "select_oneobstypearea");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, latmax, "select_oneobstypearea");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, presmin, "select_oneobstypearea");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, presmax, "select_oneobstypearea");
...
Sel_V_select_oneobstypearea(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk9real *TmpVec_4 = UseDSlong(P->T_hdr, ECMA, pk9real, P->T_hdr->lon);
    const pk9real *TmpVec_6 = UseDSlong(P->T_hdr, ECMA, pk9real, P->T_hdr->lat);
    const pk9real *TmpVec_8 = UseDSlong(P->T_body, ECMA, pk9real, P->T_body->vertco_reference_1);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, select_oneobstypearea, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_oneobstypearea, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_oneobstypearea, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_oneobstypearea, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_oneobstypearea(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_oneobstypearea", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$varno", "select_oneobstypearea", it, USD_varno_ECMA);
    Pool->add_var("ECMA", "$lonmin", "select_oneobstypearea", it, USD_lonmin_ECMA);
    Pool->add_var("ECMA", "$lonmax", "select_oneobstypearea", it, USD_lonmax_ECMA);
    Pool->add_var("ECMA", "$latmin", "select_oneobstypearea", it, USD_latmin_ECMA);
    Pool->add_var("ECMA", "$latmax", "select_oneobstypearea", it, USD_latmax_ECMA);
    Pool->add_var("ECMA", "$presmin", "select_oneobstypearea", it, USD_presmin_ECMA);
    Pool->add_var("ECMA", "$presmax", "select_oneobstypearea", it, USD_presmax_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_select_oneobstypearea(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_oneobstypearea *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_oneobstypearea);
