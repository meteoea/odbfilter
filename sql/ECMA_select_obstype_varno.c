  ...
  const pk1int T_bodyXvarno[])
{
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_obstype_varno");
  ODBMAC_LC_GETVAL(ECMA, vno, "select_obstype_varno");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_obstype_varno");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vno, "select_obstype_varno");
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, select_obstype_varno, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_obstype_varno, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_obstype_varno, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_obstype_varno, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_obstype_varno(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_obstype_varno", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$vno", "select_obstype_varno", it, USD_vno_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_select_obstype_varno(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_obstype_varno *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_obstype_varno);
