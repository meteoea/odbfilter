...
#define BODY_Datum_status_Active_EXPR(i) GET_BITS(T_bodyXdatum_status[i], 0, 1) /* 'Bitfield:datum_status.active@body' */
...
typedef struct {
  ...
  ODBMAC_VIEW_TABLEDECL_NO_INDEX(body);
  uint can_UPDATE[RNDUP_DIV(1,MAXBITS)];
} VIEW_update_hdr_status;

PRIVATE int nV_update_hdr_status_TAG = 2;
PRIVATE int nV_update_hdr_status_MEM = 0;

PRIVATE const ODB_Tags V_update_hdr_status_TAG[2] = {
  /* === SELECT-symbols (count = 1) === */
  { "Bitfield:report_status.active@hdr", 0, 0, NULL } ,
  /* === Symbols for auxiliary columns (count = 0) === */
  /* === Symbols in SELECT-expressions (count = 0) === */
  /* === WHERE-symbols (count = 1) === */
  { "Bitfield:datum_status.active@body", 0, 0, NULL },
  /* === ORDERBY-symbols (count = 0) === */
  /* === UNIQUEBY-symbols (count = 0) === */
};

PRIVATE int nV_update_hdr_status_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_update_hdr_status_PREPTAG[3] = {
  /* Prepared tags for faster codb_getnames() */
  { (preptag_name | preptag_extname), 26,
    ";report_status.active@hdr;" },
  { (preptag_type | preptag_exttype), 10,
    ";Bitfield;" },
  {  preptag_tblname, 12,
    ";@hdr;@body;" },
};
...
PRIVATE void
Ccl_V_update_hdr_status(void *T)
{
  ODBMAC_CCL_V_PRE(update_hdr_status);
  FREEINDEX(hdr);
}

PRIVATE int
PrS_V_update_hdr_status(FILE *do_trace,
  VIEW_update_hdr_status *P, int it, ODB_PE_Info *PEinfo
  ...
  const Bitfield T_bodyXdatum_status[])
{
  ...
  DRHOOK_START(PrS_V_update_hdr_status);
  ODBMAC_PEINFO_SETUP();
  for (PE=PEstart; PE<=PEend; PE++) {
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      ...
      for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'body' : weight = 361.000382 */
        double tmp1 = BODY_Datum_status_Active_EXPR(K1);
        int cond1 = (tmp1 == ((double)1));
        if (cond1 != 0)
        { /* if-block start */
...
PRIVATE int
PoS_V_update_hdr_status(FILE *do_trace,
  const VIEW_update_hdr_status *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, const unsigned int BmapIdx0[], int Index_hdr[]
  /* TABLE 'body' */ , int N1, const unsigned int BmapIdx1[],
  const Bitfield T_bodyXdatum_status[])
{
  double *Addr = (PEinfo && P->USD_symbols > 0) ? PEinfo->addr : NULL;
  int PE, PEstart = 1;
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_update_hdr_status);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
...
PRIVATE int
Sel_V_update_hdr_status(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_update_hdr_status *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(update_hdr_status, "hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_update_hdr_status);
  ...
  { /* Start of pre- & post-select block */
    const Bitfield *TmpVec_1 = UseDSlong(P->T_body, CCMA, Bitfield, P->T_body->datum_status);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_update_hdr_status(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_update_hdr_status(do_trace, P, it, PEinfo
  ...
  ODBMAC_ERRMSG_SELVIEW(update_hdr_status);
  ODB_debug_print_index(stdout, "update_hdr_status", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_update_hdr_status(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_update_hdr_status);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, update_hdr_status, Bitfield, K1, K2, hdr, D, report_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_update_hdr_status(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_update_hdr_status);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, update_hdr_status, Bitfield, K1, K2, hdr, report_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_update_hdr_status(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(update_hdr_status); }

#define Swapout_V_update_hdr_status Ccl_V_update_hdr_status

PRIVATE int
Sql_V_update_hdr_status(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_update_hdr_status(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 1;
  static const int ColAux[1] = {1};
  ODBMAC_COPY_COLAUX(update_hdr_status);
  return filled;
}

PRIVATE int *
SortKeys_V_update_hdr_status(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(update_hdr_status); }

PRIVATE int 
UpdateInfo_V_update_hdr_status(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(update_hdr_status); }

PRIVATE int *
GetIndex_V_update_hdr_status(void *V, const char *Table, int *Nidx)
{
  VIEW_update_hdr_status *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_update_hdr_status(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_update_hdr_status(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(update_hdr_status); }

PRIVATE void *
Init_V_update_hdr_status(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_update_hdr_status *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_update_hdr_status);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 1;
  P->Nrows = 0;
  ...
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 0, 0);   /* Bitfield:report_status.active@hdr */
  {
    ODB_Pool *p = Pool;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "update_hdr_status"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_update_hdr_status_TAG;
    pfcom->preptags = V_update_hdr_status_PREPTAG;
    pfcom->ntag = nV_update_hdr_status_TAG;
    pfcom->npreptag = nV_update_hdr_status_PREPTAG;
    pfcom->nmem = nV_update_hdr_status_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_update_hdr_status;
    pfcom->swapout = Swapout_V_update_hdr_status;
    pfcom->dim = Dim_V_update_hdr_status;
    pfcom->sortkeys = SortKeys_V_update_hdr_status;
    pfcom->update_info = UpdateInfo_V_update_hdr_status;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_update_hdr_status;
    pfcom->putindex = PutIndex_V_update_hdr_status;
    pfcom->peinfo = PEinfo_V_update_hdr_status;
    pfcom->select = Sel_V_update_hdr_status;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_update_hdr_status;
    pfcom->dget = dGet_V_update_hdr_status;
    pfcom->dput = dPut_V_update_hdr_status;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_update_hdr_status;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_update_hdr_status;
...
/* *************** End of VIEW "update_hdr_status" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_update_hdr_status(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_update_hdr_status *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_update_hdr_status);
  if (!P) P = Init_V_update_hdr_status(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
