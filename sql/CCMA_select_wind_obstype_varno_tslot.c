...
PRIVATE double USD_vno_u_CCMA = 0; /* $vno_u */
PRIVATE double USD_vno_v_CCMA = 0; /* $vno_v */
PRIVATE double USD_tslot_CCMA = 0; /* $tslot */
  ...
  const pk1int T_indexXtimeslot[])
{
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(CCMA, otype, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(CCMA, vno_u, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(CCMA, vno_v, "select_wind_obstype_varno_tslot");
  ODBMAC_LC_GETVAL(CCMA, tslot, "select_wind_obstype_varno_tslot");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, otype, "select_wind_obstype_varno_tslot");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, vno_u, "select_wind_obstype_varno_tslot");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, vno_v, "select_wind_obstype_varno_tslot");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, tslot, "select_wind_obstype_varno_tslot");
...
Sel_V_select_wind_obstype_varno_tslot(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    const pk1int *TmpVec_7 = UseDSlong(P->T_hdr, CCMA, pk1int, P->T_hdr->obstype);
    const pk1int *TmpVec_8 = UseDSlong(P->T_body, CCMA, pk1int, P->T_body->varno);
    const pk1int *TmpVec_10 = UseDSlong(P->T_index, CCMA, pk1int, P->T_index->timeslot);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(CCMA, d, 1, select_wind_obstype_varno_tslot, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_wind_obstype_varno_tslot, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  Call_GatherGet_VIEW(CCMA, d, 3, select_wind_obstype_varno_tslot, string, K1, K2, hdr, D, statid, DATATYPE_STRING, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 4, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, D, lat, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 5, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, D, lon, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 6, select_wind_obstype_varno_tslot, yyyymmdd, K1, K2, hdr, D, date, DATATYPE_YYYYMMDD, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 7, select_wind_obstype_varno_tslot, hhmmss, K1, K2, hdr, D, time, DATATYPE_HHMMSS, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_wind_obstype_varno_tslot, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_wind_obstype_varno_tslot, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  Call_ScatterPut_VIEW(CCMA, d, 3, select_wind_obstype_varno_tslot, string, K1, K2, hdr, statid, D, DATATYPE_STRING, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 4, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, lat, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 5, select_wind_obstype_varno_tslot, pk9real, K1, K2, hdr, lon, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 6, select_wind_obstype_varno_tslot, yyyymmdd, K1, K2, hdr, date, D, DATATYPE_YYYYMMDD, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 7, select_wind_obstype_varno_tslot, hhmmss, K1, K2, hdr, time, D, DATATYPE_HHMMSS, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_wind_obstype_varno_tslot(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("CCMA", "$otype", "select_wind_obstype_varno_tslot", it, USD_otype_CCMA);
    Pool->add_var("CCMA", "$vno_u", "select_wind_obstype_varno_tslot", it, USD_vno_u_CCMA);
    Pool->add_var("CCMA", "$vno_v", "select_wind_obstype_varno_tslot", it, USD_vno_v_CCMA);
    Pool->add_var("CCMA", "$tslot", "select_wind_obstype_varno_tslot", it, USD_tslot_CCMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2CCMA_select_wind_obstype_varno_tslot(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_wind_obstype_varno_tslot *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_wind_obstype_varno_tslot);
