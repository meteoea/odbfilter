...
PRIVATE double USD_ident_ECMA = 0; /* $ident */
PRIVATE double USD_distance_ECMA = 0; /* $distance */
PRIVATE double USD_elev_ECMA = 0; /* $elev */
PRIVATE double USD_azimut_ECMA = 0; /* $azimut */
  ...
  const pk9real T_radar_bodyXazimuth[])
{
  ...
  linkoffset_t *Psat_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(sat));
  linklen_t *Psat_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(sat));
  linkoffset_t *Pradar_station_off = UseDSlong(P->T_sat, ECMA, linkoffset_t, P->T_sat->LINKOFFSET(radar_station));
  linklen_t *Pradar_station_len = UseDSlong(P->T_sat, ECMA, linklen_t, P->T_sat->LINKLEN(radar_station));
  linkoffset_t *Pradar_off = UseDSlong(P->T_sat, ECMA, linkoffset_t, P->T_sat->LINKOFFSET(radar));
  linklen_t *Pradar_len = UseDSlong(P->T_sat, ECMA, linklen_t, P->T_sat->LINKLEN(radar));
  linkoffset_t *Pradar_body_off = UseDSlong(P->T_radar, ECMA, linkoffset_t, P->T_radar->LINKOFFSET(radar_body));
  linklen_t *Pradar_body_len = UseDSlong(P->T_radar, ECMA, linklen_t, P->T_radar->LINKLEN(radar_body));
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_oneobs_radar");
  ODBMAC_LC_GETVAL(ECMA, vno, "select_oneobs_radar");
  ODBMAC_LC_GETVAL(ECMA, ident, "select_oneobs_radar");
  ODBMAC_LC_GETVAL(ECMA, distance, "select_oneobs_radar");
  ODBMAC_LC_GETVAL(ECMA, elev, "select_oneobs_radar");
  ODBMAC_LC_GETVAL(ECMA, azimut, "select_oneobs_radar");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_oneobs_radar");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vno, "select_oneobs_radar");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, ident, "select_oneobs_radar");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, distance, "select_oneobs_radar");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, elev, "select_oneobs_radar");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, azimut, "select_oneobs_radar");
...
Sel_V_select_oneobs_radar(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_4 = UseDSlong(P->T_radar_station, ECMA, pk1int, P->T_radar_station->ident);
    const pk9real *TmpVec_5 = UseDSlong(P->T_radar_body, ECMA, pk9real, P->T_radar_body->distance);
    const pk9real *TmpVec_7 = UseDSlong(P->T_radar_body, ECMA, pk9real, P->T_radar_body->elevation);
    const pk9real *TmpVec_9 = UseDSlong(P->T_radar_body, ECMA, pk9real, P->T_radar_body->azimuth);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, select_oneobs_radar, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_oneobs_radar, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_oneobs_radar, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_oneobs_radar, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_oneobs_radar(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_oneobs_radar", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$vno", "select_oneobs_radar", it, USD_vno_ECMA);
    Pool->add_var("ECMA", "$ident", "select_oneobs_radar", it, USD_ident_ECMA);
    Pool->add_var("ECMA", "$distance", "select_oneobs_radar", it, USD_distance_ECMA);
    Pool->add_var("ECMA", "$elev", "select_oneobs_radar", it, USD_elev_ECMA);
    Pool->add_var("ECMA", "$azimut", "select_oneobs_radar", it, USD_azimut_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_select_oneobs_radar(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_oneobs_radar *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_oneobs_radar);
