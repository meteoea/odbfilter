...
#define HDR_Sensor_all_EXPR(i) T_hdrXsensor[i] /* 'pk1int:sensor@hdr' */
...
} VIEW_select_obstype_sensor;

PRIVATE int nV_select_obstype_sensor_TAG = 4;
PRIVATE int nV_select_obstype_sensor_MEM = 0;

PRIVATE const ODB_Tags V_select_obstype_sensor_TAG[4] = {
  ...
  { "pk1int:sensor@hdr", 0, 0, NULL },
  /* === ORDERBY-symbols (count = 0) === */
  /* === UNIQUEBY-symbols (count = 0) === */
};

PRIVATE int nV_select_obstype_sensor_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_obstype_sensor_PREPTAG[3] = {
...
PRIVATE void
Ccl_V_select_obstype_sensor(void *T)
{
  ODBMAC_CCL_V_PRE(select_obstype_sensor);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_obstype_sensor(FILE *do_trace,
  VIEW_select_obstype_sensor *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, unsigned int BmapIdx0[]
  /* TABLE 'body' */ , int N1, unsigned int BmapIdx1[],
  const pk1int T_hdrXobstype[],
  const pk1int T_hdrXsensor[])
{
  ...
  DRHOOK_START(PrS_V_select_obstype_sensor);
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_obstype_sensor");
  ODBMAC_LC_GETVAL(ECMA, vno, "select_obstype_sensor");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_obstype_sensor");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vno, "select_obstype_sensor");
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      ...
      int K1;
      double tmp2 = HDR_Obstype_all_EXPR(K0);
      double tmp3 = HDR_Sensor_all_EXPR(K0);
      int cond0 = (tmp2 == lc_USD_otype)
            && (tmp3 == lc_USD_vno);
      if (cond0 != 0)
      { /* if-block start */
        tmpcount += Pbody_len[K0]; /* TABLE 'body' : weight = 361.000382 */
      } /* if-block end */
    } /* TABLE 'hdr' */
...
PRIVATE int
PoS_V_select_obstype_sensor(FILE *do_trace,
  const VIEW_select_obstype_sensor *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk1int T_hdrXsensor[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_obstype_sensor);
  ODBMAC_LC_GETVAL(ECMA, otype, "select_obstype_sensor");
  ODBMAC_LC_GETVAL(ECMA, vno, "select_obstype_sensor");
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
...
PRIVATE int
Sel_V_select_obstype_sensor(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_obstype_sensor *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_obstype_sensor, "hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_obstype_sensor);
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_3 = UseDSlong(P->T_hdr, ECMA, pk1int, P->T_hdr->sensor);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_obstype_sensor(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_obstype_sensor(do_trace, P, it, PEinfo
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
  ...
  ODBMAC_ERRMSG_SELVIEW(select_obstype_sensor);
  ODB_debug_print_index(stdout, "select_obstype_sensor", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_obstype_sensor(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_obstype_sensor);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(ECMA, d, 1, select_obstype_sensor, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_obstype_sensor, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_obstype_sensor(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_obstype_sensor);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_obstype_sensor, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_obstype_sensor, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_obstype_sensor(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_obstype_sensor); }

#define Swapout_V_select_obstype_sensor Ccl_V_select_obstype_sensor

PRIVATE int
Sql_V_select_obstype_sensor(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_obstype_sensor(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_obstype_sensor);
  return filled;
}

PRIVATE int *
SortKeys_V_select_obstype_sensor(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_obstype_sensor); }

PRIVATE int 
UpdateInfo_V_select_obstype_sensor(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_obstype_sensor); }

PRIVATE int *
GetIndex_V_select_obstype_sensor(void *V, const char *Table, int *Nidx)
{
  VIEW_select_obstype_sensor *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_obstype_sensor(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_obstype_sensor(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_obstype_sensor); }

PRIVATE void *
Init_V_select_obstype_sensor(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_obstype_sensor *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_obstype_sensor);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_obstype_sensor"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_obstype_sensor_TAG;
    pfcom->preptags = V_select_obstype_sensor_PREPTAG;
    pfcom->ntag = nV_select_obstype_sensor_TAG;
    pfcom->npreptag = nV_select_obstype_sensor_PREPTAG;
    pfcom->nmem = nV_select_obstype_sensor_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_obstype_sensor;
    pfcom->swapout = Swapout_V_select_obstype_sensor;
    pfcom->dim = Dim_V_select_obstype_sensor;
    pfcom->sortkeys = SortKeys_V_select_obstype_sensor;
    pfcom->update_info = UpdateInfo_V_select_obstype_sensor;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_obstype_sensor;
    pfcom->putindex = PutIndex_V_select_obstype_sensor;
    pfcom->peinfo = PEinfo_V_select_obstype_sensor;
    pfcom->select = Sel_V_select_obstype_sensor;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_obstype_sensor;
    pfcom->dget = dGet_V_select_obstype_sensor;
    pfcom->dput = dPut_V_select_obstype_sensor;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_obstype_sensor;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_obstype_sensor;
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_obstype_sensor", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$vno", "select_obstype_sensor", it, USD_vno_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}

/* *************** End of VIEW "select_obstype_sensor" *************** */

PUBLIC ODB_Funcs *
Anchor2ECMA_select_obstype_sensor(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_obstype_sensor *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_obstype_sensor);
  if (!P) P = Init_V_select_obstype_sensor(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
