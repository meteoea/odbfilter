  ...
  const pk1int T_indexXtimeslot[])
{
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(CCMA, tslot, "select_scat_tslot");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, tslot, "select_scat_tslot");
...
Sel_V_select_scat_tslot(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_4 = UseDSlong(P->T_body, CCMA, pk1int, P->T_body->varno);
    const pk1int *TmpVec_6 = UseDSlong(P->T_index, CCMA, pk1int, P->T_index->timeslot);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(CCMA, d, 1, select_scat_tslot, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_scat_tslot, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  Call_GatherGet_VIEW(CCMA, d, 3, select_scat_tslot, pk1int, K1, K2, hdr, D, seqno, DATATYPE_INT4, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_scat_tslot, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_scat_tslot, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  Call_ScatterPut_VIEW(CCMA, d, 3, select_scat_tslot, pk1int, K1, K2, hdr, seqno, D, DATATYPE_INT4, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_scat_tslot(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("CCMA", "$tslot", "select_scat_tslot", it, USD_tslot_CCMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2CCMA_select_scat_tslot(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_scat_tslot *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_scat_tslot);
