...
#define ODB_CONSIDER_TABLES "/hdr/body/"

#define BODY_Datum_status_Passive_EXPR(i) GET_BITS(T_bodyXdatum_status[i], 1, 1) /* 'Bitfield:datum_status.passive@body' */
#define HDR_Obstype_all_EXPR(i) T_hdrXobstype[i] /* 'pk1int:obstype@hdr' */
#define HDR_Codetype_all_EXPR(i) T_hdrXcodetype[i] /* 'pk1int:codetype@hdr' */
...
PRIVATE double USD_ctype_CCMA = 0; /* $ctype */
PRIVATE double USD_lon_CCMA = 0; /* $lon */
PRIVATE double USD_lat_CCMA = 0; /* $lat */


#if !defined(K0_lo_var) && !defined(K0_lo_const)
#define K0_lo 0
#endif
#define hdr_ROW ((double)(K0 + 1))
#define body_ROW ((double)(K1 + 1))

/* *************** VIEW "select_oneobs_gpssol" *************** */
...
} VIEW_select_oneobs_gpssol;

PRIVATE int nV_select_oneobs_gpssol_TAG = 7;
PRIVATE int nV_select_oneobs_gpssol_MEM = 0;

PRIVATE const ODB_Tags V_select_oneobs_gpssol_TAG[7] = {
  ...
  { "Bitfield:datum_status.passive@body", 0, 0, NULL },
  { "pk1int:obstype@hdr", 0, 0, NULL },
  { "pk1int:codetype@hdr", 0, 0, NULL },
...
PRIVATE int nV_select_oneobs_gpssol_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_oneobs_gpssol_PREPTAG[3] = {
...
PRIVATE void
Ccl_V_select_oneobs_gpssol(void *T)
{
  ODBMAC_CCL_V_PRE(select_oneobs_gpssol);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_oneobs_gpssol(FILE *do_trace,
  VIEW_select_oneobs_gpssol *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, unsigned int BmapIdx0[]
  /* TABLE 'body' */ , int N1, unsigned int BmapIdx1[],
  const Bitfield T_bodyXdatum_status[],
  const pk1int T_hdrXobstype[],
  const pk1int T_hdrXcodetype[],
  const pk9real T_hdrXlon[],
  const pk9real T_hdrXlat[])
{
  double lc_USD_otype = 0;
  double lc_USD_ctype = 0;
  double lc_USD_lon = 0;
  double lc_USD_lat = 0;
  ...
  DRHOOK_START(PrS_V_select_oneobs_gpssol);
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(CCMA, otype, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(CCMA, ctype, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(CCMA, lon, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(CCMA, lat, "select_oneobs_gpssol");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, otype, "select_oneobs_gpssol");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, ctype, "select_oneobs_gpssol");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, lon, "select_oneobs_gpssol");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, lat, "select_oneobs_gpssol");
      if (!Addr_trigger) Addr = NULL;
    }
    ODBMAC_PEINFO_SKIP();
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      int K1_lo = Pbody_off[K0];
      int K1_hi = K1_lo + Pbody_len[K0];
      int K1;
      double tmp3 = HDR_Obstype_all_EXPR(K0);
      double tmp4 = HDR_Codetype_all_EXPR(K0);
      double tmp5 = HDR_Lon_all_EXPR(K0);
      double tmp6 = tmp5;
      double tmp7 = HDR_Lat_all_EXPR(K0);
      double tmp8 = tmp7;
      int cond0 = (tmp3 == lc_USD_otype)
            && (tmp4 == lc_USD_ctype)
            && (tmp5 > (lc_USD_lon - ((double)0.001)))
            && (tmp6 < (lc_USD_lon + ((double)0.001)))
            && (tmp7 > (lc_USD_lat - ((double)0.001)))
            && (tmp8 < (lc_USD_lat + ((double)0.001)));
      if (cond0 == 0) continue;
      for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'body' : weight = 361.000382 */
        double tmp2 = BODY_Datum_status_Passive_EXPR(K1);
        int cond1 = (tmp2 == ((double)0));
        if (cond1 != 0)
        { /* if-block start */
...
PRIVATE int
PoS_V_select_oneobs_gpssol(FILE *do_trace,
  const VIEW_select_oneobs_gpssol *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, const unsigned int BmapIdx0[]
  /* TABLE 'body' */ , int N1, const unsigned int BmapIdx1[], int Index_body[],
  const Bitfield T_bodyXdatum_status[],
  const pk1int T_hdrXobstype[],
  const pk1int T_hdrXcodetype[],
  const pk9real T_hdrXlon[],
  const pk9real T_hdrXlat[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_oneobs_gpssol);
  ODBMAC_LC_GETVAL(CCMA, otype, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(CCMA, ctype, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(CCMA, lon, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(CCMA, lat, "select_oneobs_gpssol");
...
PRIVATE int
Sel_V_select_oneobs_gpssol(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_oneobs_gpssol *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_oneobs_gpssol, "hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_oneobs_gpssol);
  ...
  { /* Start of pre- & post-select block */
    const Bitfield *TmpVec_2 = UseDSlong(P->T_body, CCMA, Bitfield, P->T_body->datum_status);
    const pk1int *TmpVec_3 = UseDSlong(P->T_hdr, CCMA, pk1int, P->T_hdr->obstype);
    const pk1int *TmpVec_4 = UseDSlong(P->T_hdr, CCMA, pk1int, P->T_hdr->codetype);
    const pk9real *TmpVec_5 = UseDSlong(P->T_hdr, CCMA, pk9real, P->T_hdr->lon);
    const pk9real *TmpVec_7 = UseDSlong(P->T_hdr, CCMA, pk9real, P->T_hdr->lat);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_oneobs_gpssol(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_oneobs_gpssol(do_trace, P, it, PEinfo
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
  ...
  ODBMAC_ERRMSG_SELVIEW(select_oneobs_gpssol);
  ODB_debug_print_index(stdout, "select_oneobs_gpssol", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_oneobs_gpssol(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_oneobs_gpssol);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, select_oneobs_gpssol, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_oneobs_gpssol, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_oneobs_gpssol(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_oneobs_gpssol);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_oneobs_gpssol, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_oneobs_gpssol, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_oneobs_gpssol(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_oneobs_gpssol); }

#define Swapout_V_select_oneobs_gpssol Ccl_V_select_oneobs_gpssol

PRIVATE int
Sql_V_select_oneobs_gpssol(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_oneobs_gpssol(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_oneobs_gpssol);
  return filled;
}

PRIVATE int *
SortKeys_V_select_oneobs_gpssol(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_oneobs_gpssol); }

PRIVATE int 
UpdateInfo_V_select_oneobs_gpssol(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_oneobs_gpssol); }

PRIVATE int *
GetIndex_V_select_oneobs_gpssol(void *V, const char *Table, int *Nidx)
{
  VIEW_select_oneobs_gpssol *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_oneobs_gpssol(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_oneobs_gpssol(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_oneobs_gpssol); }

PRIVATE void *
Init_V_select_oneobs_gpssol(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_oneobs_gpssol *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_oneobs_gpssol);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  P->USD_symbols = 4; /* In SELECT = 0 ; In WHERE = 6 */
  P->Replicate_PE = 0;
  P->Npes = 0;
  P->NrowVec = NULL;
  P->NrowOffset = NULL;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_oneobs_gpssol"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_oneobs_gpssol_TAG;
    pfcom->preptags = V_select_oneobs_gpssol_PREPTAG;
    pfcom->ntag = nV_select_oneobs_gpssol_TAG;
    pfcom->npreptag = nV_select_oneobs_gpssol_PREPTAG;
    pfcom->nmem = nV_select_oneobs_gpssol_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_oneobs_gpssol;
    pfcom->swapout = Swapout_V_select_oneobs_gpssol;
    pfcom->dim = Dim_V_select_oneobs_gpssol;
    pfcom->sortkeys = SortKeys_V_select_oneobs_gpssol;
    pfcom->update_info = UpdateInfo_V_select_oneobs_gpssol;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_oneobs_gpssol;
    pfcom->putindex = PutIndex_V_select_oneobs_gpssol;
    pfcom->peinfo = PEinfo_V_select_oneobs_gpssol;
    pfcom->select = Sel_V_select_oneobs_gpssol;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_oneobs_gpssol;
    pfcom->dget = dGet_V_select_oneobs_gpssol;
    pfcom->dput = dPut_V_select_oneobs_gpssol;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_oneobs_gpssol;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_oneobs_gpssol;
  ...
  if (add_vars) {
    Pool->add_var("CCMA", "$otype", "select_oneobs_gpssol", it, USD_otype_CCMA);
    Pool->add_var("CCMA", "$ctype", "select_oneobs_gpssol", it, USD_ctype_CCMA);
    Pool->add_var("CCMA", "$lon", "select_oneobs_gpssol", it, USD_lon_CCMA);
    Pool->add_var("CCMA", "$lat", "select_oneobs_gpssol", it, USD_lat_CCMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}

/* *************** End of VIEW "select_oneobs_gpssol" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_select_oneobs_gpssol(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_oneobs_gpssol *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_oneobs_gpssol);
  if (!P) P = Init_V_select_oneobs_gpssol(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
