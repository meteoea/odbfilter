  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(CCMA, d, 1, hdr_set_passive, Bitfield, K1, K2, hdr, D, report_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, hdr_set_passive, Bitfield, K1, K2, hdr, report_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
PUBLIC ODB_Funcs *
Anchor2CCMA_hdr_set_passive(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_hdr_set_passive *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_hdr_set_passive);
