...
#define ODB_CONSIDER_TABLES "/timeslot_index/index/hdr/body/"

#define HDR_Obstype_all_EXPR(i) T_hdrXobstype[i] /* 'pk1int:obstype@hdr' */
#define TIMESLOT_INDEX_Timeslot_all_EXPR(i) T_timeslot_indexXtimeslot[i] /* 'pk1int:timeslot@timeslot_index' */
...
#define K0_lo 0
#endif
#define timeslot_index_ROW ((double)(K0 + 1))
#define index_ROW ((double)(K1 + 1))
#define hdr_ROW ((double)(K2 + 1))
#define body_ROW ((double)(K3 + 1))

/* *************** VIEW "select_obstype_wind" *************** */
...
typedef struct {
  ...
  int  NSortKeys;
  int *SortKeys;
  ODBMAC_VIEW_TABLEDECL_NO_INDEX(timeslot_index);
...
} VIEW_select_obstype_wind;

PRIVATE int nV_select_obstype_wind_TAG = 6;
PRIVATE int nV_select_obstype_wind_MEM = 0;

PRIVATE const ODB_Tags V_select_obstype_wind_TAG[6] = {
  ...
  /* === WHERE-symbols (count = 4) === */
  { "pk1int:obstype@hdr", 0, 0, NULL },
  { "pk1int:timeslot@timeslot_index", 0, 0, NULL },
...
PRIVATE int nV_select_obstype_wind_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_obstype_wind_PREPTAG[3] = {
  ...
  { (preptag_type | preptag_exttype), 18,
    ";pk9real;Bitfield;" },
  {  preptag_tblname, 35,
    ";@timeslot_index;@index;@hdr;@body;" },
};

PRIVATE void
Ccl_V_select_obstype_wind(void *T)
{
  ODBMAC_CCL_V_PRE(select_obstype_wind);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_obstype_wind(FILE *do_trace,
  VIEW_select_obstype_wind *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'timeslot_index' */ , int N0, unsigned int BmapIdx0[]
  /* TABLE 'index' */ , int N1, unsigned int BmapIdx1[]
  /* TABLE 'hdr' */ , int N2, unsigned int BmapIdx2[]
  /* TABLE 'body' */ , int N3, unsigned int BmapIdx3[],
  const pk1int T_hdrXobstype[],
  const pk1int T_timeslot_indexXtimeslot[],
  ...
  const pk1int T_bodyXvarno[])
{
  ...
  int K0_hi = N0;
  int Count = 0;
  linkoffset_t *Pindex_off = UseDSlong(P->T_timeslot_index, CCMA, linkoffset_t, P->T_timeslot_index->LINKOFFSET(index));
  linklen_t *Pindex_len = UseDSlong(P->T_timeslot_index, CCMA, linklen_t, P->T_timeslot_index->LINKLEN(index));
  ...
  DRHOOK_START(PrS_V_select_obstype_wind);
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(CCMA, otype, "select_obstype_wind");
  ODBMAC_LC_GETVAL(CCMA, tslot, "select_obstype_wind");
  ODBMAC_LC_GETVAL(CCMA, vno_u, "select_obstype_wind");
  ODBMAC_LC_GETVAL(CCMA, vno_v, "select_obstype_wind");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, otype, "select_obstype_wind");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, tslot, "select_obstype_wind");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, vno_u, "select_obstype_wind");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, vno_v, "select_obstype_wind");
      if (!Addr_trigger) Addr = NULL;
    }
    ODBMAC_PEINFO_SKIP();
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'timeslot_index' : weight = 1.000002 */
      int K1_lo = Pindex_off[K0];
      int K1_hi = K1_lo + Pindex_len[K0];
      int K1;
      double tmp3 = TIMESLOT_INDEX_Timeslot_all_EXPR(K0);
      int cond0 = (tmp3 == lc_USD_tslot);
      if (cond0 == 0) continue;
      for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'index' : weight = 1.000003 */
        int K2_lo = Phdr_off[K1];
        int K2;
        K2 = K2_lo; { /* TABLE 'hdr' (due to ONELOOPER with 'index') : weight = 3.000004 */
          int K3_lo = Pbody_off[K2];
          int K3_hi = K3_lo + Pbody_len[K2];
          int K3;
          double tmp2 = HDR_Obstype_all_EXPR(K2);
          int cond2 = (tmp2 == lc_USD_otype);
          if (cond2 == 0) continue;
          for (K3=K3_lo; K3<K3_hi; K3++) { /* TABLE 'body' : weight = 361.000382 */
            double tmp4 = BODY_Vertco_reference_1_all_EXPR(K3);
            double tmp5 = tmp4;
            double tmp6 = BODY_Varno_all_EXPR(K3);
            int cond3 = (tmp4 > ((double)98000))
            && (tmp5 < ((double)102700))
            && ((tmp6 == lc_USD_vno_u) || 
            (tmp6 == lc_USD_vno_v));
            if (cond3 != 0)
...
PRIVATE int
PoS_V_select_obstype_wind(FILE *do_trace,
  const VIEW_select_obstype_wind *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'timeslot_index' */ , int N0, const unsigned int BmapIdx0[]
  /* TABLE 'index' */ , int N1, const unsigned int BmapIdx1[]
  /* TABLE 'hdr' */ , int N2, const unsigned int BmapIdx2[]
  /* TABLE 'body' */ , int N3, const unsigned int BmapIdx3[], int Index_body[],
  const pk1int T_hdrXobstype[],
  const pk1int T_timeslot_indexXtimeslot[],
  const pk9real T_bodyXvertco_reference_1[],
  const pk1int T_bodyXvarno[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_obstype_wind);
  ODBMAC_LC_GETVAL(CCMA, otype, "select_obstype_wind");
  ODBMAC_LC_GETVAL(CCMA, tslot, "select_obstype_wind");
  ODBMAC_LC_GETVAL(CCMA, vno_u, "select_obstype_wind");
  ODBMAC_LC_GETVAL(CCMA, vno_v, "select_obstype_wind");
...
PRIVATE int
Sel_V_select_obstype_wind(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_obstype_wind *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_obstype_wind, "timeslot_index,index,hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_obstype_wind);
  ODBMAC_PEINFO_SELVIEW_SETUP();
  FREEINDEX(body);
  ODBMAC_TRACE_SELVIEW_PRE();
  ODBMAC_VIEW_DELAYED_LOAD(timeslot_index);
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_3 = UseDSlong(P->T_timeslot_index, CCMA, pk1int, P->T_timeslot_index->timeslot);
    const pk9real *TmpVec_4 = UseDSlong(P->T_body, CCMA, pk9real, P->T_body->vertco_reference_1);
    const pk1int *TmpVec_6 = UseDSlong(P->T_body, CCMA, pk1int, P->T_body->varno);
    ALLOCBITMAPINDEX(timeslot_index);
    ALLOCBITMAPINDEX(index);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_obstype_wind(do_trace, P, it, PEinfo
      , P->T_timeslot_index->Nrows, P->BitmapIndex_timeslot_index
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_obstype_wind(do_trace, P, it, PEinfo
        , P->T_timeslot_index->Nrows, P->BitmapIndex_timeslot_index
        , P->T_index->Nrows, P->BitmapIndex_index
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
    ...
    FREEBITMAPINDEX(timeslot_index);
    FREEBITMAPINDEX(index);
    FREEBITMAPINDEX(hdr);
    FREEBITMAPINDEX(body);
  } /* End of pre-& post-select block */
  ODBMAC_TRACE_SELVIEW_LAST();
  ODBMAC_ERRMSG_SELVIEW(select_obstype_wind);
  ODB_debug_print_index(stdout, "select_obstype_wind", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_obstype_wind(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_obstype_wind);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, select_obstype_wind, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_obstype_wind, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_obstype_wind(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_obstype_wind);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_obstype_wind, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_obstype_wind, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_obstype_wind(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_obstype_wind); }

#define Swapout_V_select_obstype_wind Ccl_V_select_obstype_wind

PRIVATE int
Sql_V_select_obstype_wind(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_obstype_wind(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_obstype_wind);
  return filled;
}

PRIVATE int *
SortKeys_V_select_obstype_wind(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_obstype_wind); }

PRIVATE int 
UpdateInfo_V_select_obstype_wind(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_obstype_wind); }

PRIVATE int *
GetIndex_V_select_obstype_wind(void *V, const char *Table, int *Nidx)
{
  VIEW_select_obstype_wind *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_obstype_wind(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_obstype_wind(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_obstype_wind); }

PRIVATE void *
Init_V_select_obstype_wind(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_obstype_wind *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_obstype_wind);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  {
    ODB_Pool *p = Pool;
    ODBMAC_ASSIGN_TABLEDATA(timeslot_index);
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_obstype_wind"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_obstype_wind_TAG;
    pfcom->preptags = V_select_obstype_wind_PREPTAG;
    pfcom->ntag = nV_select_obstype_wind_TAG;
    pfcom->npreptag = nV_select_obstype_wind_PREPTAG;
    pfcom->nmem = nV_select_obstype_wind_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_obstype_wind;
    pfcom->swapout = Swapout_V_select_obstype_wind;
    pfcom->dim = Dim_V_select_obstype_wind;
    pfcom->sortkeys = SortKeys_V_select_obstype_wind;
    pfcom->update_info = UpdateInfo_V_select_obstype_wind;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_obstype_wind;
    pfcom->putindex = PutIndex_V_select_obstype_wind;
    pfcom->peinfo = PEinfo_V_select_obstype_wind;
    pfcom->select = Sel_V_select_obstype_wind;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_obstype_wind;
    pfcom->dget = dGet_V_select_obstype_wind;
    pfcom->dput = dPut_V_select_obstype_wind;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_obstype_wind;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_obstype_wind;
  ...
  if (add_vars) {
    Pool->add_var("CCMA", "$otype", "select_obstype_wind", it, USD_otype_CCMA);
    Pool->add_var("CCMA", "$tslot", "select_obstype_wind", it, USD_tslot_CCMA);
    Pool->add_var("CCMA", "$vno_u", "select_obstype_wind", it, USD_vno_u_CCMA);
    Pool->add_var("CCMA", "$vno_v", "select_obstype_wind", it, USD_vno_v_CCMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}

/* *************** End of VIEW "select_obstype_wind" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_select_obstype_wind(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_obstype_wind *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_obstype_wind);
  if (!P) P = Init_V_select_obstype_wind(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
