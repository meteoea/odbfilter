  ...
  const pk1int T_hdrXsensor[])
{
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(CCMA, otype, "select_obstype_sensor");
  ODBMAC_LC_GETVAL(CCMA, vno, "select_obstype_sensor");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, otype, "select_obstype_sensor");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, vno, "select_obstype_sensor");
...
Sel_V_select_obstype_sensor(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_3 = UseDSlong(P->T_hdr, CCMA, pk1int, P->T_hdr->sensor);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(CCMA, d, 1, select_obstype_sensor, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_obstype_sensor, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_obstype_sensor, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_obstype_sensor, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_obstype_sensor(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("CCMA", "$otype", "select_obstype_sensor", it, USD_otype_CCMA);
    Pool->add_var("CCMA", "$vno", "select_obstype_sensor", it, USD_vno_CCMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2CCMA_select_obstype_sensor(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_obstype_sensor *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_obstype_sensor);
