...
#define ODB_CONSIDER_TABLES "/hdr/"
...
} VIEW_hdr_set_passive;

PRIVATE int nV_hdr_set_passive_TAG = 1;
PRIVATE int nV_hdr_set_passive_MEM = 0;

PRIVATE const ODB_Tags V_hdr_set_passive_TAG[1] = {
...
PRIVATE int nV_hdr_set_passive_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_hdr_set_passive_PREPTAG[3] = {
...
PRIVATE void
Ccl_V_hdr_set_passive(void *T)
{
  ODBMAC_CCL_V_PRE(hdr_set_passive);
  FREEINDEX(hdr);
}

PRIVATE int
PrS_V_hdr_set_passive(FILE *do_trace,
  VIEW_hdr_set_passive *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, unsigned int BmapIdx0[])
{
  ...
  int K0_hi = N0;
  int Count = 0;
  DRHOOK_START(PrS_V_hdr_set_passive);
  ODBMAC_PEINFO_SETUP();
  for (PE=PEstart; PE<=PEend; PE++) {
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      { /* if-block start */
          tmpcount++; /* TABLE 'hdr' : weight = 3.000004 */
      } /* if-block end */
    } /* TABLE 'hdr' */
...
PRIVATE int
PoS_V_hdr_set_passive(FILE *do_trace,
  const VIEW_hdr_set_passive *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, const unsigned int BmapIdx0[], int Index_hdr[])
{
  ...
  int Count = 0;
  int tmpcount = 0;
  DRHOOK_START(PoS_V_hdr_set_passive);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
...
PRIVATE int
Sel_V_hdr_set_passive(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_hdr_set_passive *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(hdr_set_passive, "hdr");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_hdr_set_passive);
...
PRIVATE int
dGet_V_hdr_set_passive(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_hdr_set_passive);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(ECMA, d, 1, hdr_set_passive, Bitfield, K1, K2, hdr, D, report_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_hdr_set_passive(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_hdr_set_passive);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, hdr_set_passive, Bitfield, K1, K2, hdr, report_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_hdr_set_passive(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(hdr_set_passive); }

#define Swapout_V_hdr_set_passive Ccl_V_hdr_set_passive

PRIVATE int
Sql_V_hdr_set_passive(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_hdr_set_passive(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 1;
  static const int ColAux[1] = {1};
  ODBMAC_COPY_COLAUX(hdr_set_passive);
  return filled;
}

PRIVATE int *
SortKeys_V_hdr_set_passive(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(hdr_set_passive); }

PRIVATE int 
UpdateInfo_V_hdr_set_passive(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(hdr_set_passive); }

PRIVATE int *
GetIndex_V_hdr_set_passive(void *V, const char *Table, int *Nidx)
{
  VIEW_hdr_set_passive *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_hdr_set_passive(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_hdr_set_passive(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(hdr_set_passive); }

PRIVATE void *
Init_V_hdr_set_passive(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_hdr_set_passive *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_hdr_set_passive);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 1;
  P->Nrows = 0;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "hdr_set_passive"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_hdr_set_passive_TAG;
    pfcom->preptags = V_hdr_set_passive_PREPTAG;
    pfcom->ntag = nV_hdr_set_passive_TAG;
    pfcom->npreptag = nV_hdr_set_passive_PREPTAG;
    pfcom->nmem = nV_hdr_set_passive_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_hdr_set_passive;
    pfcom->swapout = Swapout_V_hdr_set_passive;
    pfcom->dim = Dim_V_hdr_set_passive;
    pfcom->sortkeys = SortKeys_V_hdr_set_passive;
    pfcom->update_info = UpdateInfo_V_hdr_set_passive;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_hdr_set_passive;
    pfcom->putindex = PutIndex_V_hdr_set_passive;
    pfcom->peinfo = PEinfo_V_hdr_set_passive;
    pfcom->select = Sel_V_hdr_set_passive;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_hdr_set_passive;
    pfcom->dget = dGet_V_hdr_set_passive;
    pfcom->dput = dPut_V_hdr_set_passive;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_hdr_set_passive;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_hdr_set_passive;
...
/* *************** End of VIEW "hdr_set_passive" *************** */

PUBLIC ODB_Funcs *
Anchor2ECMA_hdr_set_passive(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_hdr_set_passive *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_hdr_set_passive);
  if (!P) P = Init_V_hdr_set_passive(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
