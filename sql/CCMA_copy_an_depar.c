...
PRIVATE int nV_copy_an_depar_TAG = 2;
PRIVATE int nV_copy_an_depar_MEM = 0;

PRIVATE const ODB_Tags V_copy_an_depar_TAG[2] = {
  /* === SELECT-symbols (count = 2) === */
  { "pk9real:an_depar@body", 0, 0, NULL } ,
...
PRIVATE int nV_copy_an_depar_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_copy_an_depar_PREPTAG[3] = {
  /* Prepared tags for faster codb_getnames() */
  { (preptag_name | preptag_extname), 30,
    ";an_depar@body;hires@update_2;" },
...
PRIVATE void
Ccl_V_copy_an_depar(void *T)
{
  ODBMAC_CCL_V_PRE(copy_an_depar);
  FREEINDEX(body);
  FREEINDEX(update_2);
}

PRIVATE int
PrS_V_copy_an_depar(FILE *do_trace,
  VIEW_copy_an_depar *P, int it, ODB_PE_Info *PEinfo
  ...
  /* TABLE 'update_2' */ , int N2, unsigned int BmapIdx2[])
{
  ...
  DRHOOK_START(PrS_V_copy_an_depar);
  ODBMAC_PEINFO_SETUP();
...
PRIVATE int
PoS_V_copy_an_depar(FILE *do_trace,
  const VIEW_copy_an_depar *P, int it, ODB_PE_Info *PEinfo
  ...
  /* TABLE 'update_2' */ , int N2, const unsigned int BmapIdx2[], int Index_update_2[])
{
  ...
  linkoffset_t *Pupdate_2_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(update_2));
  linklen_t *Pupdate_2_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(update_2));
  DRHOOK_START(PoS_V_copy_an_depar);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
...
PRIVATE int
Sel_V_copy_an_depar(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_copy_an_depar *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(copy_an_depar, "hdr,body,update_2");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_copy_an_depar);
  ...
  { /* Start of pre- & post-select block */
    ...
    CountPrS = PrS_V_copy_an_depar(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_copy_an_depar(do_trace, P, it, PEinfo
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
  ...
  ODBMAC_ERRMSG_SELVIEW(copy_an_depar);
  ODB_debug_print_index(stdout, "copy_an_depar", P->PoolNo, CountPrS, 2
...
PRIVATE int
dGet_V_copy_an_depar(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_copy_an_depar);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, copy_an_depar, pk9real, K1, K2, body, D, an_depar, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, copy_an_depar, pk9real, K1, K2, update_2, D, hires, DATATYPE_REAL8, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_copy_an_depar(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_copy_an_depar);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, copy_an_depar, pk9real, K1, K2, body, an_depar, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, copy_an_depar, pk9real, K1, K2, update_2, hires, D, DATATYPE_REAL8, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_copy_an_depar(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(copy_an_depar); }

#define Swapout_V_copy_an_depar Ccl_V_copy_an_depar

PRIVATE int
Sql_V_copy_an_depar(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_copy_an_depar(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(copy_an_depar);
  return filled;
}

PRIVATE int *
SortKeys_V_copy_an_depar(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(copy_an_depar); }

PRIVATE int 
UpdateInfo_V_copy_an_depar(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(copy_an_depar); }

PRIVATE int *
GetIndex_V_copy_an_depar(void *V, const char *Table, int *Nidx)
{
  VIEW_copy_an_depar *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_copy_an_depar(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_copy_an_depar(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(copy_an_depar); }

PRIVATE void *
Init_V_copy_an_depar(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_copy_an_depar *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_copy_an_depar);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 0, 0);   /* pk9real:an_depar@body */
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 1, 1);   /* pk9real:hires@update_2 */
  {
    ODB_Pool *p = Pool;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "copy_an_depar"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_copy_an_depar_TAG;
    pfcom->preptags = V_copy_an_depar_PREPTAG;
    pfcom->ntag = nV_copy_an_depar_TAG;
    pfcom->npreptag = nV_copy_an_depar_PREPTAG;
    pfcom->nmem = nV_copy_an_depar_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_copy_an_depar;
    pfcom->swapout = Swapout_V_copy_an_depar;
    pfcom->dim = Dim_V_copy_an_depar;
    pfcom->sortkeys = SortKeys_V_copy_an_depar;
    pfcom->update_info = UpdateInfo_V_copy_an_depar;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_copy_an_depar;
    pfcom->putindex = PutIndex_V_copy_an_depar;
    pfcom->peinfo = PEinfo_V_copy_an_depar;
    pfcom->select = Sel_V_copy_an_depar;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_copy_an_depar;
    pfcom->dget = dGet_V_copy_an_depar;
    pfcom->dput = dPut_V_copy_an_depar;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_copy_an_depar;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_copy_an_depar;
...
/* *************** End of VIEW "copy_an_depar" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_copy_an_depar(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_copy_an_depar *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_copy_an_depar);
  if (!P) P = Init_V_copy_an_depar(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
