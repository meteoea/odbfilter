...
Sel_V_select_amsua(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_4 = UseDSlong(P->T_hdr, CCMA, pk1int, P->T_hdr->sensor);
    const pk9real *TmpVec_5 = UseDSlong(P->T_body, CCMA, pk9real, P->T_body->vertco_reference_1);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(CCMA, d, 1, select_amsua, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_amsua, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_amsua, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_amsua, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
PUBLIC ODB_Funcs *
Anchor2CCMA_select_amsua(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_amsua *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_amsua);
