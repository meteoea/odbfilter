...
#define ODB_CONSIDER_TABLES "/hdr/body/update_2/"
...
#define update_2_ROW ((double)(K2 + 1))

/* *************** VIEW "select_for_debug" *************** */
...
typedef struct {
  ...
  ODBMAC_VIEW_TABLEDECL_WITH_INDEX(update_2);
  uint can_UPDATE[RNDUP_DIV(2,MAXBITS)];
} VIEW_select_for_debug;

PRIVATE int nV_select_for_debug_TAG = 2;
PRIVATE int nV_select_for_debug_MEM = 0;

PRIVATE const ODB_Tags V_select_for_debug_TAG[2] = {
  /* === SELECT-symbols (count = 2) === */
  { "pk9real:hires@update_2", 0, 0, NULL } ,
...
PRIVATE int nV_select_for_debug_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_for_debug_PREPTAG[3] = {
  /* Prepared tags for faster codb_getnames() */
  { (preptag_name | preptag_extname), 41,
    ";hires@update_2;datum_status.active@body;" },
...
PRIVATE void
Ccl_V_select_for_debug(void *T)
{
  ODBMAC_CCL_V_PRE(select_for_debug);
  FREEINDEX(body);
  FREEINDEX(update_2);
}

PRIVATE int
PrS_V_select_for_debug(FILE *do_trace,
  VIEW_select_for_debug *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, unsigned int BmapIdx0[]
  /* TABLE 'body' */ , int N1, unsigned int BmapIdx1[]
  /* TABLE 'update_2' */ , int N2, unsigned int BmapIdx2[])
{
  ...
  linkoffset_t *Pupdate_2_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(update_2));
  linklen_t *Pupdate_2_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(update_2));
  DRHOOK_START(PrS_V_select_for_debug);
  ODBMAC_PEINFO_SETUP();
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      ...
      { /* if-block start */
        for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'body' : weight = 361.000382 */
          int K2_lo = Pupdate_2_off[K0];
          int K2;
              tmpcount++; /* TABLE 'update_2' : weight = 361.000385 */
        } /* TABLE 'body' */
      } /* if-block end */
    } /* TABLE 'hdr' */
...
PRIVATE int
PoS_V_select_for_debug(FILE *do_trace,
  const VIEW_select_for_debug *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'hdr' */ , int N0, const unsigned int BmapIdx0[]
  /* TABLE 'body' */ , int N1, const unsigned int BmapIdx1[], int Index_body[]
  /* TABLE 'update_2' */ , int N2, const unsigned int BmapIdx2[], int Index_update_2[])
{
  double *Addr = (PEinfo && P->USD_symbols > 0) ? PEinfo->addr : NULL;
  int PE, PEstart = 1;
  int PEend = PEinfo ? PEinfo->npes : PEstart;
  ...
  linkoffset_t *Pupdate_2_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(update_2));
  linklen_t *Pupdate_2_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(update_2));
  DRHOOK_START(PoS_V_select_for_debug);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      ...
      { /* if-block start */
        for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'body' : weight = 361.000382 */
          int K2_lo = Pupdate_2_off[K0];
          int K2 = K2_lo;
          K2 = K2_lo + (K1 - K1_lo); { /* TABLE 'update_2' (due to ALIGN with 'body') : weight = 361.000385 */
            Index_body[tmpcount] = K1;
            Index_update_2[tmpcount] = K2;
            tmpcount++;
          } /* TABLE 'update_2' */
        } /* TABLE 'body' */
      } /* if-block end */
...
PRIVATE int
Sel_V_select_for_debug(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_for_debug *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_for_debug, "hdr,body,update_2");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_for_debug);
  ...
  ODBMAC_VIEW_DELAYED_LOAD(update_2);
  ODBMAC_TRACE_SELVIEW_0();
  ODBMAC_TRACE_SELVIEW_1();
  { /* Start of pre- & post-select block */
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    ALLOCBITMAPINDEX(update_2);
    CountPrS = PrS_V_select_for_debug(do_trace, P, it, PEinfo
      , P->T_hdr->Nrows, P->BitmapIndex_hdr
      , P->T_body->Nrows, P->BitmapIndex_body
      , P->T_update_2->Nrows, P->BitmapIndex_update_2);
    ODBMAC_TRACE_SELVIEW_POST();
    ALLOCINDEX(body, CountPrS);
    ALLOCINDEX(update_2, CountPrS);
    ODBMAC_TRACE_SELVIEW_2();
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_for_debug(do_trace, P, it, PEinfo
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
        , P->T_body->Nrows, P->BitmapIndex_body, P->Index_body
        , P->T_update_2->Nrows, P->BitmapIndex_update_2, P->Index_update_2); }
    else { CountPoS = CountPrS; }
    FREEBITMAPINDEX(hdr);
    FREEBITMAPINDEX(body);
    FREEBITMAPINDEX(update_2);
  } /* End of pre-& post-select block */
  ODBMAC_TRACE_SELVIEW_LAST();
  ODBMAC_ERRMSG_SELVIEW(select_for_debug);
  ODB_debug_print_index(stdout, "select_for_debug", P->PoolNo, CountPrS, 2
        , "body", P->Index_body, P->T_body, P->T_body->Nrows
        , "update_2", P->Index_update_2, P->T_update_2, P->T_update_2->Nrows);
  P->Nrows = CountPrS;
  DRHOOK_END(CountPrS);
  return CountPrS;
}
...
PRIVATE int
dGet_V_select_for_debug(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_for_debug);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, select_for_debug, pk9real, K1, K2, update_2, D, hires, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_for_debug, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_for_debug(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_for_debug);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_for_debug, pk9real, K1, K2, update_2, hires, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_for_debug, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_for_debug(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_for_debug); }

#define Swapout_V_select_for_debug Ccl_V_select_for_debug

PRIVATE int
Sql_V_select_for_debug(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_for_debug(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_for_debug);
  return filled;
}

PRIVATE int *
SortKeys_V_select_for_debug(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_for_debug); }

PRIVATE int 
UpdateInfo_V_select_for_debug(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_for_debug); }

PRIVATE int *
GetIndex_V_select_for_debug(void *V, const char *Table, int *Nidx)
{
  VIEW_select_for_debug *P = V;
  int Dummy = 0;
  int *Nlen = Nidx ? Nidx : &Dummy;
  ODBMAC_GETINDEX(body);
  ODBMAC_GETINDEX(update_2);
  return NULL;
}

PRIVATE int
PutIndex_V_select_for_debug(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
  /* *** Warning: This is a VERY DANGEROUS routine -- if misused !!! */
  VIEW_select_for_debug *P = V;
  int rc = 0;
  ODBMAC_PUTINDEX(body);
  ODBMAC_PUTINDEX(update_2);
  return rc;
}

PRIVATE void
PEinfo_V_select_for_debug(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_for_debug); }

PRIVATE void *
Init_V_select_for_debug(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_for_debug *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_for_debug);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 0, 0);   /* pk9real:hires@update_2 */
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 1, 1);   /* Bitfield:datum_status.active@body */
  {
    ...
    ODBMAC_ASSIGN_TABLEDATA(update_2);
  }
  NULLIFY_INDEX(body);
  NULLIFY_INDEX(update_2);
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_for_debug"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_for_debug_TAG;
    pfcom->preptags = V_select_for_debug_PREPTAG;
    pfcom->ntag = nV_select_for_debug_TAG;
    pfcom->npreptag = nV_select_for_debug_PREPTAG;
    pfcom->nmem = nV_select_for_debug_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_for_debug;
    pfcom->swapout = Swapout_V_select_for_debug;
    pfcom->dim = Dim_V_select_for_debug;
    pfcom->sortkeys = SortKeys_V_select_for_debug;
    pfcom->update_info = UpdateInfo_V_select_for_debug;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_for_debug;
    pfcom->putindex = PutIndex_V_select_for_debug;
    pfcom->peinfo = PEinfo_V_select_for_debug;
    pfcom->select = Sel_V_select_for_debug;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_for_debug;
    pfcom->dget = dGet_V_select_for_debug;
    pfcom->dput = dPut_V_select_for_debug;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_for_debug;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_for_debug;
...
/* *************** End of VIEW "select_for_debug" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_select_for_debug(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_for_debug *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_for_debug);
  if (!P) P = Init_V_select_for_debug(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
