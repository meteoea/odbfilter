  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, delete_passive, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, delete_passive, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_delete_passive(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_delete_passive *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_delete_passive);
