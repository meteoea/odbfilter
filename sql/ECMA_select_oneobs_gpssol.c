...
PRIVATE double USD_ctype_ECMA = 0; /* $ctype */
PRIVATE double USD_lon_ECMA = 0; /* $lon */
PRIVATE double USD_lat_ECMA = 0; /* $lat */
  ...
  const pk9real T_hdrXlat[])
{
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(ECMA, ctype, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(ECMA, lon, "select_oneobs_gpssol");
  ODBMAC_LC_GETVAL(ECMA, lat, "select_oneobs_gpssol");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_oneobs_gpssol");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, ctype, "select_oneobs_gpssol");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, lon, "select_oneobs_gpssol");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, lat, "select_oneobs_gpssol");
...
Sel_V_select_oneobs_gpssol(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    const Bitfield *TmpVec_2 = UseDSlong(P->T_body, ECMA, Bitfield, P->T_body->datum_status);
    const pk1int *TmpVec_3 = UseDSlong(P->T_hdr, ECMA, pk1int, P->T_hdr->obstype);
    const pk1int *TmpVec_4 = UseDSlong(P->T_hdr, ECMA, pk1int, P->T_hdr->codetype);
    const pk9real *TmpVec_5 = UseDSlong(P->T_hdr, ECMA, pk9real, P->T_hdr->lon);
    const pk9real *TmpVec_7 = UseDSlong(P->T_hdr, ECMA, pk9real, P->T_hdr->lat);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, select_oneobs_gpssol, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_oneobs_gpssol, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_oneobs_gpssol, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_oneobs_gpssol, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_oneobs_gpssol(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_oneobs_gpssol", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$ctype", "select_oneobs_gpssol", it, USD_ctype_ECMA);
    Pool->add_var("ECMA", "$lon", "select_oneobs_gpssol", it, USD_lon_ECMA);
    Pool->add_var("ECMA", "$lat", "select_oneobs_gpssol", it, USD_lat_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_select_oneobs_gpssol(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_oneobs_gpssol *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_oneobs_gpssol);
