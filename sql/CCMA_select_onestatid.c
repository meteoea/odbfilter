...
#define HDR_Statid_all_EXPR(i) T_hdrXstatid[i] /* 'string:statid@hdr' */


PRIVATE double USD_statid_CCMA = 0; /* $statid */
...
} VIEW_select_onestatid;

PRIVATE int nV_select_onestatid_TAG = 3;
PRIVATE int nV_select_onestatid_MEM = 0;

PRIVATE const ODB_Tags V_select_onestatid_TAG[3] = {
...
PRIVATE int nV_select_onestatid_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_onestatid_PREPTAG[3] = {
...
PRIVATE void
Ccl_V_select_onestatid(void *T)
{
  ODBMAC_CCL_V_PRE(select_onestatid);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_onestatid(FILE *do_trace,
  VIEW_select_onestatid *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'index' */ , int N0, unsigned int BmapIdx0[]
  /* TABLE 'hdr' */ , int N1, unsigned int BmapIdx1[]
  /* TABLE 'body' */ , int N2, unsigned int BmapIdx2[],
  const string T_hdrXstatid[])
{
  double lc_USD_statid = 0;
  ...
  DRHOOK_START(PrS_V_select_onestatid);
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(CCMA, statid, "select_onestatid");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, statid, "select_onestatid");
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'index' : weight = 1.000003 */
      ...
      K1 = K1_lo; { /* TABLE 'hdr' (due to ONELOOPER with 'index') : weight = 3.000004 */
        ...
        int K2;
        double tmp2 = HDR_Statid_all_EXPR(K1);
        int cond1 = (tmp2 == lc_USD_statid);
        if (cond1 != 0)
        { /* if-block start */
          tmpcount += Pbody_len[K1]; /* TABLE 'body' : weight = 361.000382 */
        } /* if-block end */
      } /* TABLE 'hdr' */
    } /* TABLE 'index' */
...
PRIVATE int
PoS_V_select_onestatid(FILE *do_trace,
  const VIEW_select_onestatid *P, int it, ODB_PE_Info *PEinfo
  ...
  const string T_hdrXstatid[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_onestatid);
  ODBMAC_LC_GETVAL(CCMA, statid, "select_onestatid");
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
...
PRIVATE int
Sel_V_select_onestatid(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_onestatid *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_onestatid, "index,hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_onestatid);
  ...
  { /* Start of pre- & post-select block */
    const string *TmpVec_2 = UseDSlong(P->T_hdr, CCMA, string, P->T_hdr->statid);
    ALLOCBITMAPINDEX(index);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_onestatid(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_onestatid(do_trace, P, it, PEinfo
        , P->T_index->Nrows, P->BitmapIndex_index
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
  ...
  ODBMAC_ERRMSG_SELVIEW(select_onestatid);
  ODB_debug_print_index(stdout, "select_onestatid", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_onestatid(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_onestatid);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, select_onestatid, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_onestatid, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_onestatid(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_onestatid);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_onestatid, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_onestatid, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_onestatid(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_onestatid); }

#define Swapout_V_select_onestatid Ccl_V_select_onestatid

PRIVATE int
Sql_V_select_onestatid(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_onestatid(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_onestatid);
  return filled;
}

PRIVATE int *
SortKeys_V_select_onestatid(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_onestatid); }

PRIVATE int 
UpdateInfo_V_select_onestatid(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_onestatid); }

PRIVATE int *
GetIndex_V_select_onestatid(void *V, const char *Table, int *Nidx)
{
  VIEW_select_onestatid *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_onestatid(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_onestatid(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_onestatid); }

PRIVATE void *
Init_V_select_onestatid(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_onestatid *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_onestatid);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_onestatid"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_onestatid_TAG;
    pfcom->preptags = V_select_onestatid_PREPTAG;
    pfcom->ntag = nV_select_onestatid_TAG;
    pfcom->npreptag = nV_select_onestatid_PREPTAG;
    pfcom->nmem = nV_select_onestatid_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_onestatid;
    pfcom->swapout = Swapout_V_select_onestatid;
    pfcom->dim = Dim_V_select_onestatid;
    pfcom->sortkeys = SortKeys_V_select_onestatid;
    pfcom->update_info = UpdateInfo_V_select_onestatid;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_onestatid;
    pfcom->putindex = PutIndex_V_select_onestatid;
    pfcom->peinfo = PEinfo_V_select_onestatid;
    pfcom->select = Sel_V_select_onestatid;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_onestatid;
    pfcom->dget = dGet_V_select_onestatid;
    pfcom->dput = dPut_V_select_onestatid;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_onestatid;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_onestatid;
  ...
  if (add_vars) {
    Pool->add_var("CCMA", "$statid", "select_onestatid", it, USD_statid_CCMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}

/* *************** End of VIEW "select_onestatid" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_select_onestatid(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_onestatid *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_onestatid);
  if (!P) P = Init_V_select_onestatid(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
