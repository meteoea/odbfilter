...
PRIVATE int nV_select_amsua_TAG = 7;
PRIVATE int nV_select_amsua_MEM = 0;

PRIVATE const ODB_Tags V_select_amsua_TAG[7] = {
...
PRIVATE int nV_select_amsua_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_amsua_PREPTAG[3] = {
...
PRIVATE void
Ccl_V_select_amsua(void *T)
{
  ODBMAC_CCL_V_PRE(select_amsua);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_amsua(FILE *do_trace,
  VIEW_select_amsua *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk9real T_hdrXlat[])
{
  ...
  DRHOOK_START(PrS_V_select_amsua);
  ODBMAC_PEINFO_SETUP();
  for (PE=PEstart; PE<=PEend; PE++) {
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'index' : weight = 1.000003 */
      ...
      K1 = K1_lo; { /* TABLE 'hdr' (due to ONELOOPER with 'index') : weight = 3.000004 */
        ...
        double tmp4 = HDR_Sensor_all_EXPR(K1);
        double tmp6 = HDR_Lat_all_EXPR(K1);
        int cond1 = (tmp2 == ((double)7))
            && (tmp4 == ((double)3))
            && (Func_degrees(tmp6) > ((double)10));
        if (cond1 == 0) continue;
        for (K2=K2_lo; K2<K2_hi; K2++) { /* TABLE 'body' : weight = 361.000382 */
          double tmp3 = BODY_Varno_all_EXPR(K2);
          double tmp5 = BODY_Vertco_reference_1_all_EXPR(K2);
          int cond2 = (tmp3 == ((double)119))
            && (tmp5 == ((double)13));
          if (cond2 != 0)
          { /* if-block start */
...
PRIVATE int
PoS_V_select_amsua(FILE *do_trace,
  const VIEW_select_amsua *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk9real T_hdrXlat[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_amsua);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
...
PRIVATE int
Sel_V_select_amsua(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_amsua *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_amsua, "index,hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_amsua);
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_4 = UseDSlong(P->T_hdr, ECMA, pk1int, P->T_hdr->sensor);
    const pk9real *TmpVec_5 = UseDSlong(P->T_body, ECMA, pk9real, P->T_body->vertco_reference_1);
    const pk9real *TmpVec_6 = UseDSlong(P->T_hdr, ECMA, pk9real, P->T_hdr->lat);
    ALLOCBITMAPINDEX(index);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_amsua(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_amsua(do_trace, P, it, PEinfo
        , P->T_index->Nrows, P->BitmapIndex_index
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
  ...
  ODBMAC_ERRMSG_SELVIEW(select_amsua);
  ODB_debug_print_index(stdout, "select_amsua", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_amsua(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_amsua);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(ECMA, d, 1, select_amsua, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_amsua, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_amsua(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_amsua);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_amsua, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_amsua, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_amsua(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_amsua); }

#define Swapout_V_select_amsua Ccl_V_select_amsua

PRIVATE int
Sql_V_select_amsua(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_amsua(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_amsua);
  return filled;
}

PRIVATE int *
SortKeys_V_select_amsua(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_amsua); }

PRIVATE int 
UpdateInfo_V_select_amsua(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_amsua); }

PRIVATE int *
GetIndex_V_select_amsua(void *V, const char *Table, int *Nidx)
{
  VIEW_select_amsua *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_amsua(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_amsua(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_amsua); }

PRIVATE void *
Init_V_select_amsua(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_amsua *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_amsua);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_amsua"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_amsua_TAG;
    pfcom->preptags = V_select_amsua_PREPTAG;
    pfcom->ntag = nV_select_amsua_TAG;
    pfcom->npreptag = nV_select_amsua_PREPTAG;
    pfcom->nmem = nV_select_amsua_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_amsua;
    pfcom->swapout = Swapout_V_select_amsua;
    pfcom->dim = Dim_V_select_amsua;
    pfcom->sortkeys = SortKeys_V_select_amsua;
    pfcom->update_info = UpdateInfo_V_select_amsua;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_amsua;
    pfcom->putindex = PutIndex_V_select_amsua;
    pfcom->peinfo = PEinfo_V_select_amsua;
    pfcom->select = Sel_V_select_amsua;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_amsua;
    pfcom->dget = dGet_V_select_amsua;
    pfcom->dput = dPut_V_select_amsua;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_amsua;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_amsua;
...
/* *************** End of VIEW "select_amsua" *************** */

PUBLIC ODB_Funcs *
Anchor2ECMA_select_amsua(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_amsua *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_amsua);
  if (!P) P = Init_V_select_amsua(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
