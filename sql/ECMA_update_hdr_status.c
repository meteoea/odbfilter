...
Sel_V_update_hdr_status(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    const Bitfield *TmpVec_1 = UseDSlong(P->T_body, ECMA, Bitfield, P->T_body->datum_status);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, update_hdr_status, Bitfield, K1, K2, hdr, D, report_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, update_hdr_status, Bitfield, K1, K2, hdr, report_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_update_hdr_status(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_update_hdr_status *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_update_hdr_status);
