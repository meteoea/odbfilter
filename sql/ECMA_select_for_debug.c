  ...
  /* TABLE 'update_2' */ , int N2, unsigned int BmapIdx2[])
{
  ...
  linkoffset_t *Pupdate_2_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(update_2));
  linklen_t *Pupdate_2_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(update_2));
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, select_for_debug, pk9real, K1, K2, update_2, D, hires, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_for_debug, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_for_debug, pk9real, K1, K2, update_2, hires, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_for_debug, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_select_for_debug(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_for_debug *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_for_debug);
