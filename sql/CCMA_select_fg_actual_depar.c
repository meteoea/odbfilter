...
#define ODB_CONSIDER_TABLES "/body/"
...
#define K0_lo 0
#endif
#define body_ROW ((double)(K0 + 1))

/* *************** VIEW "select_fg_actual_depar" *************** */
...
} VIEW_select_fg_actual_depar;

PRIVATE int nV_select_fg_actual_depar_TAG = 2;
PRIVATE int nV_select_fg_actual_depar_MEM = 0;

PRIVATE const ODB_Tags V_select_fg_actual_depar_TAG[2] = {
  /* === SELECT-symbols (count = 2) === */
  { "pk9real:fg_depar@body", 0, 0, NULL } ,
  { "pk9real:actual_depar@body", 0, 0, NULL } ,
...
PRIVATE int nV_select_fg_actual_depar_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_fg_actual_depar_PREPTAG[3] = {
  /* Prepared tags for faster codb_getnames() */
  { (preptag_name | preptag_extname), 33,
    ";fg_depar@body;actual_depar@body;" },
...
PRIVATE void
Ccl_V_select_fg_actual_depar(void *T)
{
  ODBMAC_CCL_V_PRE(select_fg_actual_depar);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_fg_actual_depar(FILE *do_trace,
  VIEW_select_fg_actual_depar *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'body' */ , int N0, unsigned int BmapIdx0[])
{
  ...
  int K0_hi = N0;
  int Count = 0;
  DRHOOK_START(PrS_V_select_fg_actual_depar);
  ODBMAC_PEINFO_SETUP();
  for (PE=PEstart; PE<=PEend; PE++) {
    ...
    ODBMAC_PEINFO_SKIP();
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'body' : weight = 361.000382 */
      { /* if-block start */
...
PRIVATE int
PoS_V_select_fg_actual_depar(FILE *do_trace,
  const VIEW_select_fg_actual_depar *P, int it, ODB_PE_Info *PEinfo
  /* TABLE 'body' */ , int N0, const unsigned int BmapIdx0[], int Index_body[])
{
  double *Addr = (PEinfo && P->USD_symbols > 0) ? PEinfo->addr : NULL;
  int PE, PEstart = 1;
  int PEend = PEinfo ? PEinfo->npes : PEstart;
  ...
  int Count = 0;
  int tmpcount = 0;
  DRHOOK_START(PoS_V_select_fg_actual_depar);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
...
PRIVATE int
Sel_V_select_fg_actual_depar(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_fg_actual_depar *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_fg_actual_depar, "body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_fg_actual_depar);
  ...
  { /* Start of pre- & post-select block */
    ...
    CountPrS = PrS_V_select_fg_actual_depar(do_trace, P, it, PEinfo
      , P->T_body->Nrows, P->BitmapIndex_body);
    ODBMAC_TRACE_SELVIEW_POST();
    ALLOCINDEX(body, CountPrS);
    ODBMAC_TRACE_SELVIEW_2();
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_fg_actual_depar(do_trace, P, it, PEinfo
  ...
  ODBMAC_ERRMSG_SELVIEW(select_fg_actual_depar);
  ODB_debug_print_index(stdout, "select_fg_actual_depar", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_fg_actual_depar(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_fg_actual_depar);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, select_fg_actual_depar, pk9real, K1, K2, body, D, fg_depar, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_fg_actual_depar, pk9real, K1, K2, body, D, actual_depar, DATATYPE_REAL8, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_fg_actual_depar(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_fg_actual_depar);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_fg_actual_depar, pk9real, K1, K2, body, fg_depar, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_fg_actual_depar, pk9real, K1, K2, body, actual_depar, D, DATATYPE_REAL8, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_fg_actual_depar(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_fg_actual_depar); }

#define Swapout_V_select_fg_actual_depar Ccl_V_select_fg_actual_depar

PRIVATE int
Sql_V_select_fg_actual_depar(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_fg_actual_depar(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_fg_actual_depar);
  return filled;
}

PRIVATE int *
SortKeys_V_select_fg_actual_depar(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_fg_actual_depar); }

PRIVATE int 
UpdateInfo_V_select_fg_actual_depar(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_fg_actual_depar); }

PRIVATE int *
GetIndex_V_select_fg_actual_depar(void *V, const char *Table, int *Nidx)
{
  VIEW_select_fg_actual_depar *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_fg_actual_depar(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_fg_actual_depar(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_fg_actual_depar); }

PRIVATE void *
Init_V_select_fg_actual_depar(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_fg_actual_depar *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_fg_actual_depar);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 1, 1);   /* pk9real:actual_depar@body */
  {
    ODB_Pool *p = Pool;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_fg_actual_depar"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_fg_actual_depar_TAG;
    pfcom->preptags = V_select_fg_actual_depar_PREPTAG;
    pfcom->ntag = nV_select_fg_actual_depar_TAG;
    pfcom->npreptag = nV_select_fg_actual_depar_PREPTAG;
    pfcom->nmem = nV_select_fg_actual_depar_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_fg_actual_depar;
    pfcom->swapout = Swapout_V_select_fg_actual_depar;
    pfcom->dim = Dim_V_select_fg_actual_depar;
    pfcom->sortkeys = SortKeys_V_select_fg_actual_depar;
    pfcom->update_info = UpdateInfo_V_select_fg_actual_depar;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_fg_actual_depar;
    pfcom->putindex = PutIndex_V_select_fg_actual_depar;
    pfcom->peinfo = PEinfo_V_select_fg_actual_depar;
    pfcom->select = Sel_V_select_fg_actual_depar;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_fg_actual_depar;
    pfcom->dget = dGet_V_select_fg_actual_depar;
    pfcom->dput = dPut_V_select_fg_actual_depar;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_fg_actual_depar;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_fg_actual_depar;
...
/* *************** End of VIEW "select_fg_actual_depar" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_select_fg_actual_depar(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_fg_actual_depar *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_fg_actual_depar);
  if (!P) P = Init_V_select_fg_actual_depar(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
