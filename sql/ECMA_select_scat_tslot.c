...
typedef struct {
  ...
  uint can_UPDATE[RNDUP_DIV(3,MAXBITS)];
} VIEW_select_scat_tslot;

PRIVATE int nV_select_scat_tslot_TAG = 7;
PRIVATE int nV_select_scat_tslot_MEM = 0;

PRIVATE const ODB_Tags V_select_scat_tslot_TAG[7] = {
  /* === SELECT-symbols (count = 3) === */
  ...
  /* === ORDERBY-symbols (count = 1) === */
  { "pk1int:seqno@hdr", 0, 3, NULL },
  /* === UNIQUEBY-symbols (count = 0) === */
};

PRIVATE int nV_select_scat_tslot_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_scat_tslot_PREPTAG[3] = {
  /* Prepared tags for faster codb_getnames() */
  { (preptag_name | preptag_extname), 50,
    ";obsvalue@body;datum_status.active@body;seqno@hdr;" },
  { (preptag_type | preptag_exttype), 25,
    ";pk9real;Bitfield;pk1int;" },
  {  preptag_tblname, 19,
    ";@index;@hdr;@body;" },
};

PRIVATE void
Ccl_V_select_scat_tslot(void *T)
{
  ODBMAC_CCL_V_PRE(select_scat_tslot);
  FREEINDEX(hdr);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_scat_tslot(FILE *do_trace,
  VIEW_select_scat_tslot *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk1int T_indexXtimeslot[])
{
  ...
  DRHOOK_START(PrS_V_select_scat_tslot);
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, tslot, "select_scat_tslot");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, tslot, "select_scat_tslot");
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'index' : weight = 1.000003 */
      ...
      int K1;
      double tmp6 = INDEX_Timeslot_all_EXPR(K0);
      int cond0 = (tmp6 == lc_USD_tslot);
      if (cond0 == 0) continue;
      K1 = K1_lo; { /* TABLE 'hdr' (due to ONELOOPER with 'index') : weight = 3.000004 */
        int K2_lo = Pbody_off[K1];
        int K2_hi = K2_lo + Pbody_len[K1];
        int K2;
        double tmp3 = HDR_Obstype_all_EXPR(K1);
        int cond1 = (tmp3 == ((double)9));
        if (cond1 == 0) continue;
        for (K2=K2_lo; K2<K2_hi; K2++) { /* TABLE 'body' : weight = 361.000382 */
          double tmp4 = BODY_Varno_all_EXPR(K2);
          int cond2 = ((tmp4 == ((double)124)) || 
            (tmp4 == ((double)125)));
          if (cond2 != 0)
          { /* if-block start */
...
PRIVATE int
PoS_V_select_scat_tslot(FILE *do_trace,
  const VIEW_select_scat_tslot *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk1int T_indexXtimeslot[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_scat_tslot);
  ODBMAC_LC_GETVAL(ECMA, tslot, "select_scat_tslot");
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
...
PRIVATE int
Sel_V_select_scat_tslot(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_scat_tslot *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_scat_tslot, "index,hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_scat_tslot);
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_4 = UseDSlong(P->T_body, ECMA, pk1int, P->T_body->varno);
    const pk1int *TmpVec_6 = UseDSlong(P->T_index, ECMA, pk1int, P->T_index->timeslot);
    ALLOCBITMAPINDEX(index);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_scat_tslot(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_scat_tslot(do_trace, P, it, PEinfo
        , P->T_index->Nrows, P->BitmapIndex_index
  ...
  ODBMAC_ERRMSG_SELVIEW(select_scat_tslot);
  ODB_debug_print_index(stdout, "select_scat_tslot", P->PoolNo, CountPrS, 2
...
PRIVATE int
dGet_V_select_scat_tslot(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_scat_tslot);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(ECMA, d, 1, select_scat_tslot, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_scat_tslot, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  Call_GatherGet_VIEW(ECMA, d, 3, select_scat_tslot, pk1int, K1, K2, hdr, D, seqno, DATATYPE_INT4, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_scat_tslot(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_scat_tslot);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_scat_tslot, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_scat_tslot, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  Call_ScatterPut_VIEW(ECMA, d, 3, select_scat_tslot, pk1int, K1, K2, hdr, seqno, D, DATATYPE_INT4, 0, 0);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_scat_tslot(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_scat_tslot); }

#define Swapout_V_select_scat_tslot Ccl_V_select_scat_tslot

PRIVATE int
Sql_V_select_scat_tslot(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_scat_tslot(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 3;
  static const int ColAux[3] = {1,2,3};
  ODBMAC_COPY_COLAUX(select_scat_tslot);
  return filled;
}

PRIVATE int *
SortKeys_V_select_scat_tslot(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_scat_tslot); }

PRIVATE int 
UpdateInfo_V_select_scat_tslot(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_scat_tslot); }

PRIVATE int *
GetIndex_V_select_scat_tslot(void *V, const char *Table, int *Nidx)
{
  VIEW_select_scat_tslot *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_scat_tslot(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_scat_tslot(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_scat_tslot); }

PRIVATE void *
Init_V_select_scat_tslot(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_scat_tslot *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_scat_tslot);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 3;
  P->Nrows = 0;
  P->USD_symbols = 1; /* In SELECT = 0 ; In WHERE = 1 */
  P->Replicate_PE = 0;
  P->Npes = 0;
  P->NrowVec = NULL;
  P->NrowOffset = NULL;
  ...
  ODBIT_set(P->can_UPDATE, P->Ncols, MAXBITS, 2, 2);   /* pk1int:seqno@hdr */
  {
    ODB_Pool *p = Pool;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_scat_tslot"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_scat_tslot_TAG;
    pfcom->preptags = V_select_scat_tslot_PREPTAG;
    pfcom->ntag = nV_select_scat_tslot_TAG;
    pfcom->npreptag = nV_select_scat_tslot_PREPTAG;
    pfcom->nmem = nV_select_scat_tslot_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_scat_tslot;
    pfcom->swapout = Swapout_V_select_scat_tslot;
    pfcom->dim = Dim_V_select_scat_tslot;
    pfcom->sortkeys = SortKeys_V_select_scat_tslot;
    pfcom->update_info = UpdateInfo_V_select_scat_tslot;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_scat_tslot;
    pfcom->putindex = PutIndex_V_select_scat_tslot;
    pfcom->peinfo = PEinfo_V_select_scat_tslot;
    pfcom->select = Sel_V_select_scat_tslot;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_scat_tslot;
    pfcom->dget = dGet_V_select_scat_tslot;
    pfcom->dput = dPut_V_select_scat_tslot;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_scat_tslot;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_scat_tslot;
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$tslot", "select_scat_tslot", it, USD_tslot_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}

/* *************** End of VIEW "select_scat_tslot" *************** */

PUBLIC ODB_Funcs *
Anchor2ECMA_select_scat_tslot(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_scat_tslot *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_scat_tslot);
  if (!P) P = Init_V_select_scat_tslot(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
