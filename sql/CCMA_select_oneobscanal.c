...
#define BODY_Fg_depar_all_EXPR(i) T_bodyXfg_depar[i] /* 'pk9real:fg_depar@body' */


PRIVATE double USD_otype_CCMA = 0; /* $otype */
PRIVATE double USD_vertco_CCMA = 0; /* $vertco */
...
} VIEW_select_oneobscanal;

PRIVATE int nV_select_oneobscanal_TAG = 5;
PRIVATE int nV_select_oneobscanal_MEM = 0;

PRIVATE const ODB_Tags V_select_oneobscanal_TAG[5] = {
  ...
  { "pk9real:fg_depar@body", 0, 0, NULL },
  /* === ORDERBY-symbols (count = 0) === */
  /* === UNIQUEBY-symbols (count = 0) === */
};

PRIVATE int nV_select_oneobscanal_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_oneobscanal_PREPTAG[3] = {
...
PRIVATE void
Ccl_V_select_oneobscanal(void *T)
{
  ODBMAC_CCL_V_PRE(select_oneobscanal);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_oneobscanal(FILE *do_trace,
  VIEW_select_oneobscanal *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk9real T_bodyXfg_depar[])
{
  double lc_USD_otype = 0;
  double lc_USD_vertco = 0;
  ...
  DRHOOK_START(PrS_V_select_oneobscanal);
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(CCMA, otype, "select_oneobscanal");
  ODBMAC_LC_GETVAL(CCMA, vertco, "select_oneobscanal");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, otype, "select_oneobscanal");
      ODBMAC_ADDR_TRIGGER(CCMA, PEinfo->varname, vertco, "select_oneobscanal");
    ...
    for (K0=K0_lo; K0<K0_hi; K0++) { /* TABLE 'hdr' : weight = 3.000004 */
      ...
      for (K1=K1_lo; K1<K1_hi; K1++) { /* TABLE 'body' : weight = 361.000382 */
        double tmp3 = BODY_Vertco_reference_1_all_EXPR(K1);
        double tmp4 = BODY_Fg_depar_all_EXPR(K1);
        int cond1 = (tmp3 == lc_USD_vertco)
            && (tmp4 > ((double)3));
        if (cond1 != 0)
        { /* if-block start */
...
PRIVATE int
PoS_V_select_oneobscanal(FILE *do_trace,
  const VIEW_select_oneobscanal *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk9real T_bodyXfg_depar[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, CCMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, CCMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_oneobscanal);
  ODBMAC_LC_GETVAL(CCMA, otype, "select_oneobscanal");
  ODBMAC_LC_GETVAL(CCMA, vertco, "select_oneobscanal");
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
...
PRIVATE int
Sel_V_select_oneobscanal(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_oneobscanal *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_oneobscanal, "hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_oneobscanal);
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk9real *TmpVec_3 = UseDSlong(P->T_body, CCMA, pk9real, P->T_body->vertco_reference_1);
    const pk9real *TmpVec_4 = UseDSlong(P->T_body, CCMA, pk9real, P->T_body->fg_depar);
    ALLOCBITMAPINDEX(hdr);
    ALLOCBITMAPINDEX(body);
    CountPrS = PrS_V_select_oneobscanal(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_oneobscanal(do_trace, P, it, PEinfo
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
  ...
  ODBMAC_ERRMSG_SELVIEW(select_oneobscanal);
  ODB_debug_print_index(stdout, "select_oneobscanal", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_oneobscanal(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_oneobscanal);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(CCMA, d, 1, select_oneobscanal, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(CCMA, d, 2, select_oneobscanal, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_oneobscanal(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_oneobscanal);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(CCMA, d, 1, select_oneobscanal, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(CCMA, d, 2, select_oneobscanal, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_oneobscanal(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_oneobscanal); }

#define Swapout_V_select_oneobscanal Ccl_V_select_oneobscanal

PRIVATE int
Sql_V_select_oneobscanal(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_oneobscanal(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_oneobscanal);
  return filled;
}

PRIVATE int *
SortKeys_V_select_oneobscanal(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_oneobscanal); }

PRIVATE int 
UpdateInfo_V_select_oneobscanal(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_oneobscanal); }

PRIVATE int *
GetIndex_V_select_oneobscanal(void *V, const char *Table, int *Nidx)
{
  VIEW_select_oneobscanal *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_oneobscanal(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_oneobscanal(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_oneobscanal); }

PRIVATE void *
Init_V_select_oneobscanal(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_oneobscanal *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_oneobscanal);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_oneobscanal"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_oneobscanal_TAG;
    pfcom->preptags = V_select_oneobscanal_PREPTAG;
    pfcom->ntag = nV_select_oneobscanal_TAG;
    pfcom->npreptag = nV_select_oneobscanal_PREPTAG;
    pfcom->nmem = nV_select_oneobscanal_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_oneobscanal;
    pfcom->swapout = Swapout_V_select_oneobscanal;
    pfcom->dim = Dim_V_select_oneobscanal;
    pfcom->sortkeys = SortKeys_V_select_oneobscanal;
    pfcom->update_info = UpdateInfo_V_select_oneobscanal;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_oneobscanal;
    pfcom->putindex = PutIndex_V_select_oneobscanal;
    pfcom->peinfo = PEinfo_V_select_oneobscanal;
    pfcom->select = Sel_V_select_oneobscanal;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_oneobscanal;
    pfcom->dget = dGet_V_select_oneobscanal;
    pfcom->dput = dPut_V_select_oneobscanal;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_oneobscanal;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_oneobscanal;
  ...
  if (add_vars) {
    Pool->add_var("CCMA", "$otype", "select_oneobscanal", it, USD_otype_CCMA);
    Pool->add_var("CCMA", "$vertco", "select_oneobscanal", it, USD_vertco_CCMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}

/* *************** End of VIEW "select_oneobscanal" *************** */

PUBLIC ODB_Funcs *
Anchor2CCMA_select_oneobscanal(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_oneobscanal *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2CCMA_select_oneobscanal);
  if (!P) P = Init_V_select_oneobscanal(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
