  ...
  const pk1int T_bodyXvarno[])
{
  ...
  int K0_hi = N0;
  int Count = 0;
  linkoffset_t *Pindex_off = UseDSlong(P->T_timeslot_index, ECMA, linkoffset_t, P->T_timeslot_index->LINKOFFSET(index));
  linklen_t *Pindex_len = UseDSlong(P->T_timeslot_index, ECMA, linklen_t, P->T_timeslot_index->LINKLEN(index));
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_obstype_wind");
  ODBMAC_LC_GETVAL(ECMA, tslot, "select_obstype_wind");
  ODBMAC_LC_GETVAL(ECMA, vno_u, "select_obstype_wind");
  ODBMAC_LC_GETVAL(ECMA, vno_v, "select_obstype_wind");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_obstype_wind");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, tslot, "select_obstype_wind");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vno_u, "select_obstype_wind");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vno_v, "select_obstype_wind");
...
Sel_V_select_obstype_wind(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk1int *TmpVec_3 = UseDSlong(P->T_timeslot_index, ECMA, pk1int, P->T_timeslot_index->timeslot);
    const pk9real *TmpVec_4 = UseDSlong(P->T_body, ECMA, pk9real, P->T_body->vertco_reference_1);
    const pk1int *TmpVec_6 = UseDSlong(P->T_body, ECMA, pk1int, P->T_body->varno);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, select_obstype_wind, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_obstype_wind, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_obstype_wind, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_obstype_wind, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_obstype_wind(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_obstype_wind", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$tslot", "select_obstype_wind", it, USD_tslot_ECMA);
    Pool->add_var("ECMA", "$vno_u", "select_obstype_wind", it, USD_vno_u_ECMA);
    Pool->add_var("ECMA", "$vno_v", "select_obstype_wind", it, USD_vno_v_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_select_obstype_wind(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_obstype_wind *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_obstype_wind);
