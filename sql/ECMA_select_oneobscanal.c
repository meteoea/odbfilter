...
PRIVATE double USD_vertco_ECMA = 0; /* $vertco */
  ...
  const pk9real T_bodyXfg_depar[])
{
  ...
  ODBMAC_PEINFO_SETUP();
  ODBMAC_LC_GETVAL(ECMA, otype, "select_oneobscanal");
  ODBMAC_LC_GETVAL(ECMA, vertco, "select_oneobscanal");
  for (PE=PEstart; PE<=PEend; PE++) {
    int tmpcount = 0;
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, otype, "select_oneobscanal");
      ODBMAC_ADDR_TRIGGER(ECMA, PEinfo->varname, vertco, "select_oneobscanal");
...
Sel_V_select_oneobscanal(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  ...
  { /* Start of pre- & post-select block */
    ...
    const pk9real *TmpVec_3 = UseDSlong(P->T_body, ECMA, pk9real, P->T_body->vertco_reference_1);
    const pk9real *TmpVec_4 = UseDSlong(P->T_body, ECMA, pk9real, P->T_body->fg_depar);
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  Call_GatherGet_VIEW(ECMA, d, 1, select_oneobscanal, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_oneobscanal, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
  ...
  int ProcID, const int Flag[])
{
  ...
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_oneobscanal, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_oneobscanal, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}
...
Init_V_select_oneobscanal(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  ...
  if (add_vars) {
    Pool->add_var("ECMA", "$otype", "select_oneobscanal", it, USD_otype_ECMA);
    Pool->add_var("ECMA", "$vertco", "select_oneobscanal", it, USD_vertco_ECMA);
  } /* if (add_vars) */
  DRHOOK_END(0);
  return P;
}
...
PUBLIC ODB_Funcs *
Anchor2ECMA_select_oneobscanal(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_oneobscanal *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_oneobscanal);
