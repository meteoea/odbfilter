...
PRIVATE int nV_select_sophie_TAG = 4;
PRIVATE int nV_select_sophie_MEM = 0;

PRIVATE const ODB_Tags V_select_sophie_TAG[4] = {
...
PRIVATE int nV_select_sophie_PREPTAG = 3;
PRIVATE const ODB_PrepTags V_select_sophie_PREPTAG[3] = {
...
PRIVATE void
Ccl_V_select_sophie(void *T)
{
  ODBMAC_CCL_V_PRE(select_sophie);
  FREEINDEX(body);
}

PRIVATE int
PrS_V_select_sophie(FILE *do_trace,
  VIEW_select_sophie *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk1int T_bodyXvarno[])
{
  ...
  DRHOOK_START(PrS_V_select_sophie);
  ODBMAC_PEINFO_SETUP();
...
PRIVATE int
PoS_V_select_sophie(FILE *do_trace,
  const VIEW_select_sophie *P, int it, ODB_PE_Info *PEinfo
  ...
  const pk1int T_bodyXvarno[])
{
  ...
  linkoffset_t *Pbody_off = UseDSlong(P->T_hdr, ECMA, linkoffset_t, P->T_hdr->LINKOFFSET(body));
  linklen_t *Pbody_len = UseDSlong(P->T_hdr, ECMA, linklen_t, P->T_hdr->LINKLEN(body));
  DRHOOK_START(PoS_V_select_sophie);
  for (PE=PEstart; PE<=PEend; PE++) {
    if (Addr) {
      boolean Addr_trigger = 0;
      *Addr = PE;
...
PRIVATE int
Sel_V_select_sophie(void *V, ODB_PE_Info *PEinfo, int phase, void *feedback)
{
  VIEW_select_sophie *P = V;
  int CountPrS = 0;
  int CountPoS = 0;
  int Nbytes = 0;
  ODBMAC_TRACE_SELVIEW_SETUP(select_sophie, "hdr,body");
  int it = odb_get_thread_id();
  DRHOOK_START(Sel_V_select_sophie);
  ...
  { /* Start of pre- & post-select block */
    ...
    CountPrS = PrS_V_select_sophie(do_trace, P, it, PEinfo
    ...
    if (CountPrS > 0) {
      CountPoS = PoS_V_select_sophie(do_trace, P, it, PEinfo
        , P->T_hdr->Nrows, P->BitmapIndex_hdr
  ...
  ODBMAC_ERRMSG_SELVIEW(select_sophie);
  ODB_debug_print_index(stdout, "select_sophie", P->PoolNo, CountPrS, 1
...
PRIVATE int
dGet_V_select_sophie(void *V, double D[],
  ...
  int ProcID, const int Flag[], int row_offset)
{
  ...
  DRHOOK_START(dGet_V_select_sophie);
  ODBMAC_PEINFO_OFFSET();
  Call_GatherGet_VIEW(ECMA, d, 1, select_sophie, pk9real, K1, K2, body, D, obsvalue, DATATYPE_REAL8, 0, 0);
  Call_GatherGet_VIEW(ECMA, d, 2, select_sophie, Bitfield, K1, K2, body, D, datum_status, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE int
dPut_V_select_sophie(void *V, const double D[],
  int LdimD, int Nrows, int Ncols,
  int ProcID, const int Flag[])
{
  ...
  int Npes = P->Npes;
  FILE *do_trace = ODB_trace_fp();
  DRHOOK_START(dPut_V_select_sophie);
  ODBMAC_PEINFO_OFFSET();
  Call_ScatterPut_VIEW(ECMA, d, 1, select_sophie, pk9real, K1, K2, body, obsvalue, D, DATATYPE_REAL8, 0, 0);
  Call_ScatterPut_VIEW(ECMA, d, 2, select_sophie, Bitfield, K1, K2, body, datum_status, D, DATATYPE_BITFIELD, 0, 1);
  DRHOOK_END(K2-K1);
  return K2-K1;
}

PRIVATE void
Dim_V_select_sophie(void *V, int *Nrows, int *Ncols, int *Nrowoffset, int ProcID) { ODBMAC_DIM(select_sophie); }

#define Swapout_V_select_sophie Ccl_V_select_sophie

PRIVATE int
Sql_V_select_sophie(FILE *fp, int mode, const char *prefix, const char *postfix, char **sqlout) { ODBMAC_VIEWSQL(); }

PRIVATE int
ColAux_V_select_sophie(void *V, int colaux[], int colaux_len)
{
  int filled = 0;
  static const int ColAux_len = 2;
  static const int ColAux[2] = {1,2};
  ODBMAC_COPY_COLAUX(select_sophie);
  return filled;
}

PRIVATE int *
SortKeys_V_select_sophie(void *V, int *NSortKeys) { ODBMAC_SORTKEYS(select_sophie); }

PRIVATE int 
UpdateInfo_V_select_sophie(void *V, const int ncols, int can_UPDATE[]) { ODBMAC_UPDATEINFO(select_sophie); }

PRIVATE int *
GetIndex_V_select_sophie(void *V, const char *Table, int *Nidx)
{
  VIEW_select_sophie *P = V;
  int Dummy = 0;
...
PRIVATE int
PutIndex_V_select_sophie(void *V, const char *Table, int Nidx, int idx[], int by_address)
{
...
PRIVATE void
PEinfo_V_select_sophie(void *V, ODB_PE_Info *PEinfo) { ODBMAC_PEINFO_INIT(select_sophie); }

PRIVATE void *
Init_V_select_sophie(void *V, ODB_Pool *Pool, int Dummy1, int Dummy2, int it, int add_vars)
{
  VIEW_select_sophie *P = V;
  int PoolNo = Pool->poolno;
  ODB_Funcs *pf;
  static ODB_CommonFuncs *pfcom = NULL; /* Shared between pools & threads */
  DRHOOK_START(Init_V_select_sophie);
  if (!P) ALLOC(P, 1);
  P->PoolNo = PoolNo;
  P->Ncols = 2;
  P->Nrows = 0;
  ...
  if (!pfcom) { /* Initialize once only */
    CALLOC(pfcom,1);
    { static char s[] = "select_sophie"; pfcom->name = s; }
    pfcom->is_table = 0;
    ...
    pfcom->rank = 0;
    pfcom->wt = 0;
    pfcom->tags = V_select_sophie_TAG;
    pfcom->preptags = V_select_sophie_PREPTAG;
    pfcom->ntag = nV_select_sophie_TAG;
    pfcom->npreptag = nV_select_sophie_PREPTAG;
    pfcom->nmem = nV_select_sophie_MEM;
    pfcom->Info = NULL;
    pfcom->create_index = 0;
    pfcom->init = Init_V_select_sophie;
    pfcom->swapout = Swapout_V_select_sophie;
    pfcom->dim = Dim_V_select_sophie;
    pfcom->sortkeys = SortKeys_V_select_sophie;
    pfcom->update_info = UpdateInfo_V_select_sophie;
    pfcom->aggr_info = NULL;
    pfcom->getindex = GetIndex_V_select_sophie;
    pfcom->putindex = PutIndex_V_select_sophie;
    pfcom->peinfo = PEinfo_V_select_sophie;
    pfcom->select = Sel_V_select_sophie;
    pfcom->remove = NULL;
    pfcom->cancel = Ccl_V_select_sophie;
    pfcom->dget = dGet_V_select_sophie;
    pfcom->dput = dPut_V_select_sophie;
    pfcom->load = NULL;
    pfcom->store = NULL;
    pfcom->pack = NULL;
    pfcom->unpack = NULL;
    pfcom->sql = Sql_V_select_sophie;
    pfcom->ncols_aux = 0;
    pfcom->colaux = ColAux_V_select_sophie;
...
/* *************** End of VIEW "select_sophie" *************** */

PUBLIC ODB_Funcs *
Anchor2ECMA_select_sophie(void *V, ODB_Pool *pool, int *nviews, int it, int add_vars)
{
  VIEW_select_sophie *P = V;
  ODB_Funcs *pf;
  DRHOOK_START(Anchor2ECMA_select_sophie);
  if (!P) P = Init_V_select_sophie(NULL, pool, -1, -1, it, add_vars);
  if (nviews) *nviews = 1;
  pf = P->Funcs;
  DRHOOK_END(0);
  return pf;
